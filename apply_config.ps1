...
$sbdEn = "Basic attacks <$hpRegenIcon> <#60e84dff>heal<> the most wounded and nearest ally champion for <#d8c9b3ff>{0}<> - <#d8c9b3ff>{1}<> (based on <$levelIcon> <#d8c9b3ff>level<>) <#ff9028ff>(+{2}% <$adIcon> AD)<> <#a974ffff>(+{3}% <$apIcon> AP)<> <#60e84dff>health<>."
...
$sbdVi = "Đòn đánh thường <$hpRegenIcon> <#60e84dff>hồi<> cho tướng đồng minh bi thương nặng nhất và gần nhất <#d8c9b3ff>{0}<> - <#d8c9b3ff>{1}<> (dựa theo <$levelIcon> <#d8c9b3ff>cấp độ<>) <#ff9028ff>(+{2}% <$adIcon> SMCK)<> <#a974ffff>(+{3}% <$apIcon> SMPT)<> <#60e84dff>máu<>."
...
$sbdZh = "普通攻击<$hpRegenIcon> <#60e84dff>治疗<>最近且伤势最重的友方英雄 <#d8c9b3ff>{0}<> - <#d8c9b3ff>{1}<>（基于<$levelIcon> <#d8c9b3ff>等级<>）<#ff9028ff>（+{2}% <$adIcon> 攻击力）<> <#a974ffff>（+{3}% <$apIcon> 法术强度）<>点<#60e84dff>生命值<>。"
...
$sbdPt = "Ataques básicos <$hpRegenIcon> <#60e84dff>curam<> o campeão aliado mais ferido e mais próximo em <#d8c9b3ff>{0}<> - <#d8c9b3ff>{1}<> (com base no <$levelIcon> <#d8c9b3ff>nível<>) <#ff9028ff>(+{2}% de <$adIcon> Dano de Ataque)<> <#a974ffff>(+{3}% de <$apIcon> Poder de Habilidade)<> de <#60e84dff>Vida<>."
...
$sbdRu = "Автоатаки <$hpRegenIcon> <#60e84dff>восстанавливают<> ближайшему наиболее раненому союзному чемпиону <#d8c9b3ff>{0}<> - <#d8c9b3ff>{1}<> (в зависимости от <$levelIcon> <#d8c9b3ff>уровня<>) <#ff9028ff>(+{2}% <$adIcon> силы атаки)<> <#a974ffff>(+{3}% <$apIcon> силы умений)<> <#60e84dff>здоровья<>."
