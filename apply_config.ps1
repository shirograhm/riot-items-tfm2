...
$borkMeleePct = [double]$config.blade_of_the_ruined_king.effect_melee_hp_percent_damage
$borkRangedPct = [double]$config.blade_of_the_ruined_king.effect_ranged_hp_percent_damage
$borkCap = [int]$config.blade_of_the_ruined_king.effect_minion_damage_cap
$rborkMeleePct = [double]$config.radiant_blade_of_the_ruined_king.effect_melee_hp_percent_damage
$rborkRangedPct = [double]$config.radiant_blade_of_the_ruined_king.effect_ranged_hp_percent_damage
...
$eclMeleePct = [double]$config.eclipse.effect_melee_hp_percent_damage
$eclRangedPct = [double]$config.eclipse.effect_ranged_hp_percent_damage
...
$reclMeleePct = [double]$config.radiant_eclipse.effect_melee_hp_percent_damage
$reclRangedPct = [double]$config.radiant_eclipse.effect_ranged_hp_percent_damage
...
$bsMeleeAmp = [int]$config.bloodsong.effect_melee_damaged_amplify
$bsRangedAmp = [int]$config.bloodsong.effect_ranged_damaged_amplify
...
$rbsMeleeAmp = [int]$config.radiant_bloodsong.effect_melee_damaged_amplify
$rbsRangedAmp = [int]$config.radiant_bloodsong.effect_ranged_damaged_amplify
...
$meleeIcon = "i#asset/base/ui/banpick/champion_stat_icon:melee_0"
$rangedIcon = "i#asset/base/ui/banpick/champion_stat_icon:ranged_0"
...
$i18n.en.stormsurge.option = "Stormraider: Dealing damage to an enemy champion equal to <#60e84dff>${surgePct}%<> of their <$hpIcon> <#60e84dff>maximum health<> within <#e8a800ff>${surgeWin} seconds<> inflicts them with Squall (<#e8a800ff>${surgeCd} second<> cooldown per target).`n`nSquall: After <#e8a800ff>${surgeDelay} seconds<>, strike the target with lightning, dealing <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> of your <$apIcon> <#a974ffff>Ability Power<> as <#a974ffff>magic damage<>."
$i18n.en.radiant_stormsurge.option = "Stormraider: Dealing damage to an enemy champion equal to <#60e84dff>${rsurgePct}%<> of their <$hpIcon> <#60e84dff>maximum health<> within <#e8a800ff>${rsurgeWin} seconds<> inflicts them with Squall (<#e8a800ff>${rsurgeCd} second<> cooldown per target).`n`nSquall: After <#e8a800ff>${rsurgeDelay} seconds<>, strike the target with lightning, dealing <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> of your <$apIcon> <#a974ffff>Ability Power<> as <#a974ffff>magic damage<>."
$i18n.en.fated_ashes.option = "Inflame: Landing an Ability on an enemy burns them for <#a974ffff>${ashFlat} magic damage<> over <#e8a800ff>${ashDur} seconds<>. This effect is <#e8a800ff>${ashPct}% as effective<> against minions and monsters."
$i18n.en.hextech_alternator.option = "Revved: Damaging an enemy champion deals <#a974ffff>${altFlat} bonus magic damage<> (<#e8a800ff>${altCd} second<> cooldown)."
$i18n.en.protectors_vow.option = "Awe: Gain <$hpIcon> <#60e84dff>maximum health<> equal to <#60e84dff>${pvFlat}<> + <#ffdd8eff>${pvArmorPct}%<> of your <$armorIcon> <#ffdd8eff>armor<>."
...
$i18n.en.shadowflame.option = "Cinderbloom: Your <#a974ffff>magic<> and <#e3ab9dff>true damage<> is <#e8a800ff>20% stronger<> against enemies <#d94c49ff>below ${sfThreshold}% maximum health<>."
$i18n.en.radiant_shadowflame.option = "Cinderbloom: Your <#a974ffff>magic<> and <#e3ab9dff>true damage<> is <#e8a800ff>20% stronger<> against enemies <#d94c49ff>below ${rsfThreshold}% maximum health<>."
$i18n.en.yun_tal_wildarrows.option = "Practice Makes Lethal: Basic attacks grant <#d45656ff>${ytCrit}%<> <$critIcon> <#d45656ff>critical strike chance<> permanently, up to <#d45656ff>${ytMaxCrit}%<> <$critIcon>.`n`nFlurry: Every <#e8a800ff>${ytCd} seconds<>, your next basic attack grants <#ceff99ff>${ytFlurryAS}%<> <$asIcon> <#ceff99ff>attack speed<> for <#e8a800ff>${ytDur} seconds<>."
$i18n.en.radiant_yun_tal_wildarrows.option = "Practice Makes Lethal: Basic attacks grant <#d45656ff>${rytCrit}%<> <$critIcon> <#d45656ff>critical strike chance<> permanently, up to <#d45656ff>${rytMaxCrit}%<> <$critIcon>.`n`nFlurry: Every <#e8a800ff>${rytCd} seconds<>, your next basic attack grants <#ceff99ff>${rytFlurryAS}%<> <$asIcon> <#ceff99ff>attack speed<> for <#e8a800ff>${rytDur} seconds<>."
$i18n.en.phage.option = "Rage: Basic attacks against enemy champions grant <#ffffffff>${phageMs}%<> <$speedIcon> <#ffffffff>movement speed<> for <#e8a800ff>${phageDur} seconds<>."
...
$i18n.en.frozen_mallet.option = "Icy: Basic attacks apply a <#d94c49ff>${fmSlow}% slow<> for <#e8a800ff>${fmDur} seconds<>."
$i18n.en.radiant_frozen_mallet.option = "Icy: Basic attacks deal <#ff9028ff>bonus physical damage<> equal to <#ff9028ff>${rfmFlat}<> + <#60e84dff>${rfmHpPct}%<> of your <$hpIcon> <#60e84dff>maximum health<> and apply a <#d94c49ff>${rfmSlow}% slow<> for <#e8a800ff>${rfmDur} seconds<>."
$i18n.en.rylais_crystal_scepter.option = "Rimefrost: Landing an Ability on an enemy applies a <#d94c49ff>${rcsSlow}% slow<> for <#e8a800ff>${rcsDur} seconds<>."
$i18n.en.radiant_rylais_crystal_scepter.option = "Rimefrost: Landing an Ability on an enemy applies a <#d94c49ff>${rrcsSlow}% slow<> for <#e8a800ff>${rrcsDur} seconds<>."
...
$i18n.en.radiant_wits_end.option = "Fray: Basic attacks deal <#a974ffff>${rweDmg} bonus magic damage<>."
...
$i18n.en.blade_of_the_ruined_king.option = "Mist's Edge: Basic attacks deal <#ff9028ff>bonus physical damage<> equal to <#d94c49ff>(<$meleeIcon> ${borkMeleePct}% / <$rangedIcon> ${borkRangedPct}%)<> of the <#d94c49ff>target's current health<>. Deals a maximum of <#ff9028ff>${borkCap} physical damage<> against minions and monsters."
$i18n.en.radiant_blade_of_the_ruined_king.option = "Mist's Edge: Basic attacks deal <#ff9028ff>bonus physical damage<> equal to <#d94c49ff>(<$meleeIcon> ${rborkMeleePct}% / <$rangedIcon> ${rborkRangedPct}%)<> of the <#d94c49ff>target's current health<>. Deals a maximum of <#ff9028ff>${rborkCap} physical damage<> against minions and monsters."
...
$i18n.en.deaths_dance.option = "Ignore Pain: <#e8a800ff>${ddDelay}%<> of the damage you take is dealt over time as <#e3ab9dff>true damage<> (up to <#60e84dff>${ddBurnCap}%<> of your <$hpIcon> <#60e84dff>maximum health<> per second).`n`nDefy: Scoring a <#e8a800ff>takedown<> on an enemy champion cleanses the remaining stored damage and <$hpRegenIcon> <#60e84dff>heals you<> for <#60e84dff>${ddFlatHeal}<> + <#60e84dff>${ddHeal}%<> of your <#60e84dff>missing health<>."
$i18n.en.radiant_deaths_dance.option = "Ignore Pain: <#e8a800ff>${rddDelay}%<> of the damage you take is dealt over time as <#e3ab9dff>true damage<> (up to <#60e84dff>${rddBurnCap}%<> of your <$hpIcon> <#60e84dff>maximum health<> per second).`n`nDefy: Scoring a <#e8a800ff>takedown<> on an enemy champion cleanses the remaining stored damage and <$hpRegenIcon> <#60e84dff>heals you<> for <#60e84dff>${rddFlatHeal}<> + <#60e84dff>${rddHeal}%<> of your <#60e84dff>missing health<>."
...
$i18n.en.radiant_heartsteel.option = "Ironheart: Every <#e8a800ff>${rhsCd} seconds<>, your next basic attack deals <#ff9028ff>bonus physical damage<> equal to <#ff9028ff>${rhsFlat}<> + <#60e84dff>${rhsHpPct}%<> of your <$hpIcon> <#60e84dff>maximum health<>, granting <#60e84dff>${rhsBonusHpPct}%<> of that damage as permanent <#60e84dff>bonus health<>."
...
$i18n.en.stormrazor.option = "Energized: Moving and basic attacking generates <#92dc7bff>Energize<> stacks, up to <#e8a800ff>${srStacks}<>.`n`nBolt: When fully <#92dc7bff>Energized<>, your next basic attack deals <#a974ffff>${srDmg} bonus magic damage<> and grants you <#ffffffff>${srMs}%<> <$speedIcon> <#ffffffff>movement speed<> for <#e8a800ff>${srDur} seconds<>."
$i18n.en.radiant_stormrazor.option = "Energized: Moving and basic attacking generates <#92dc7bff>Energize<> stacks, up to <#e8a800ff>${rsrStacks}<>.`n`nBolt: When fully <#92dc7bff>Energized<>, your next basic attack deals <#a974ffff>${rsrDmg} bonus magic damage<> and grants you <#ffffffff>${rsrMs}%<> <$speedIcon> <#ffffffff>movement speed<> for <#e8a800ff>${rsrDur} seconds<>."
...
$eclEn = "Ever Rising Moon: Landing a basic attack or an Ability on an enemy champion marks them for <#e8a800ff>{0} seconds<>, up to once per cast instance. Hitting a marked champion consumes the mark to deal <#ff9028ff>bonus physical damage<> equal to <#d94c49ff>(<$meleeIcon> {1}% / <$rangedIcon> {6}%) of their maximum health<> and grant you a <#cab944ff>shield<> that absorbs <#cab944ff>{2}<> + <#ff9028ff>{3}%<> of your <$adIcon> <#ff9028ff>Attack Damage<> for <#e8a800ff>{4} seconds<> (<#e8a800ff>{5} second<> cooldown per target)."
$i18n.en.eclipse.option = $eclEn -f $eclDur, $eclMeleePct, $eclShield, $eclAdPct, $eclShDur, $eclCd, $eclRangedPct
$i18n.en.radiant_eclipse.option = $eclEn -f $reclDur, $reclMeleePct, $reclShield, $reclAdPct, $reclShDur, $reclCd, $reclRangedPct
...
$rhTemplate = "Cleave: Basic attacks deal <#ff9028ff>(<$meleeIcon> {0}% / <$rangedIcon> {1}%)<> of your <$adIcon> <#ff9028ff>Attack Damage<> as <#ff9028ff>physical damage<> to enemies within <#ff86c2ff>{2} <$rangeIcon> range<> of the target."
$i18n.en.ravenous_hydra.option = $rhTemplate -f $rhMeleePct, $rhRangedPct, $rhRange
$i18n.en.radiant_ravenous_hydra.option = $rhTemplate -f $rrhMeleePct, $rrhRangedPct, $rrhRange
...
$bbEn = "$lethEn`n`nSabotage: Scoring a <#e8a800ff>takedown<> on an enemy champion grants <#92dc7bff>Sabotage<> for <#e8a800ff>{3} seconds<>, empowering your next basic attack against a turret to deal <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> of your <$adIcon> <#ff9028ff>Attack Damage<> as <#e3ab9dff>bonus true damage<>."
...
$spfEn = "$lethEn`n`nShield Reaver: Landing a basic attack or an Ability on an enemy champion with a shield deals <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> of your <$adIcon> <#ff9028ff>Attack Damage<> as <#ff9028ff>bonus physical damage<>."
...
$oppEn = "$lethEn`n`nPreparation: After being out of combat with enemy champions for <#e8a800ff>{1} seconds<>, gain <#ffdd8eff>{2} <$armorPenIcon> Lethality<>. This bonus remains for <#e8a800ff>{3} seconds<> after dealing damage to an enemy champion."
...
$lockEn = "Devotion: <#d94c49ff>Falling below {5}% health<> grants you and all nearby allied champions a <#cab944ff>shield<> that absorbs damage equal to <#cab944ff>{6}<> - <#cab944ff>{7}<> (based on <$levelIcon> <#d8c9b3ff>level<>) <#cab944ff>health<> over <#e8a800ff>{8} seconds<> (<#e8a800ff>{9} second<> cooldown).`n`nLegion: Grant <#ffdd8eff>{0} <$armorIcon> armor<>, <#88ccffff>{1} <$mrIcon> magic resistance<>, and <#60e84dff>{2} <$hpRegenIcon> health regeneration<> to all allies within <#ff86c2ff>{4} <$rangeIcon> range<>. Minions gain <#e8a800ff>{3}%<> of this value."
...
$i18n.vi.stormsurge.option = "Đột Kích Bão Tố: Gây sát thương bằng <#60e84dff>${surgePct}%<> <$hpIcon> <#60e84dff>Máu tối đa<> của tướng địch trong vòng <#e8a800ff>${surgeWin} giây<> sẽ gây hiệu ứng Gió Giật lên chúng (hồi chiêu <#e8a800ff>${surgeCd} giây<> mỗi mục tiêu).`n`nGió Giật: Sau <#e8a800ff>${surgeDelay} giây<>, giáng sét vào mục tiêu, gây <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> <$apIcon> <#a974ffff>SMPT<> dưới dạng <#a974ffff>sát thương phép<>."
$i18n.vi.radiant_stormsurge.option = "Đột Kích Bão Tố: Gây sát thương bằng <#60e84dff>${rsurgePct}%<> <$hpIcon> <#60e84dff>Máu tối đa<> của tướng địch trong vòng <#e8a800ff>${rsurgeWin} giây<> sẽ gây hiệu ứng Gió Giật lên chúng (hồi chiêu <#e8a800ff>${rsurgeCd} giây<> mỗi mục tiêu).`n`nGió Giật: Sau <#e8a800ff>${rsurgeDelay} giây<>, giáng sét vào mục tiêu, gây <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> <$apIcon> <#a974ffff>SMPT<> dưới dạng <#a974ffff>sát thương phép<>."
$i18n.vi.fated_ashes.option = "Tàn Lửa: Kĩ năng trúng kẻ địch sẽ thiêu đốt chúng, gây <#a974ffff>${ashFlat} sát thương phép<> trong <#e8a800ff>${ashDur} giây<>. Hiệu ứng này <#e8a800ff>hiệu quả ${ashPct}%<> lên lính và quái."
$i18n.vi.hextech_alternator.option = "Căng Ga: Khi gây sát thương cho tướng địch, gây <#a974ffff>${altFlat} sát thương phép cộng thêm<> (hồi chiêu <#e8a800ff>${altCd} giây<>)."
$i18n.vi.protectors_vow.option = "Kính Sợ: Nhận <$hpIcon> <#60e84dff>máu tối đa<> tương ứng <#60e84dff>${pvFlat}<> + <#ffdd8eff>${pvArmorPct}%<> <$armorIcon> <#ffdd8eff>Giáp<> của bạn."
...
$i18n.vi.shadowflame.option = "Hạt Tro: <#a974ffff>Sát thương phép<> và <#e3ab9dff>Sát thương Chuẩn<> của bạn <#e8a800ff>mạnh hơn 20%<> khi gây lên kẻ địch <#d94c49ff>dưới ${sfThreshold}% máu tối đa<>."
$i18n.vi.radiant_shadowflame.option = "Hạt Tro: <#a974ffff>Sát thương phép<> và <#e3ab9dff>Sát thương Chuẩn<> của bạn <#e8a800ff>mạnh hơn 20%<> khi gây lên kẻ địch <#d94c49ff>dưới ${rsfThreshold}% máu tối đa<>."
$i18n.vi.yun_tal_wildarrows.option = "Chí Mạng Tay Quen: Đòn đánh thường tăng <#e8a800ff>${ytCrit}%<> <$critIcon> <#e8a800ff>tỉ lệ chí mạng<> vĩnh viễn, tối đa <#e8a800ff>${ytMaxCrit}%<>.`n`nChuyển Động Liên Hoàn: Mỗi <#e8a800ff>${ytCd} giây<>, đòn đánh thường tiếp theo cấp <#ceff99ff>${ytFlurryAS}%<> <$asIcon> <#ceff99ff>tốc độ đánh<> trong <#e8a800ff>${ytDur} giây<>."
$i18n.vi.radiant_yun_tal_wildarrows.option = "Chí Mạng Tay Quen: Đòn đánh thường tăng <#e8a800ff>${rytCrit}%<> <$critIcon> <#e8a800ff>tỉ lệ chí mạng<> vĩnh viễn, tối đa <#e8a800ff>${rytMaxCrit}%<>.`n`nChuyển Động Liên Hoàn: Mỗi <#e8a800ff>${rytCd} giây<>, đòn đánh thường tiếp theo cấp <#ceff99ff>${rytFlurryAS}%<> <$asIcon> <#ceff99ff>tốc độ đánh<> trong <#e8a800ff>${rytDur} giây<>."
$i18n.vi.phage.option = "Cuồng Nộ: Đòn đánh thường lên tướng địch sẽ cấp <#ffffffff>${phageMs}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<> trong <#e8a800ff>${phageDur} giây<>."
...
$i18n.vi.frozen_mallet.option = "Băng kết: Đòn đánh thường gây <#d94c49ff>${fmSlow}% kiệt sức <> trong <#e8a800ff>${fmDur} giây<>."
$i18n.vi.radiant_frozen_mallet.option = "Băng kết: Đòn đánh thường gây thêm <#ff9028ff>sát thương vật lí<> tương ứng <#ff9028ff>${rfmFlat}<> + <#60e84dff>${rfmHpPct}%<> <$hpIcon> <#60e84dff>máu tối đa<> của bản thân và gây <#d94c49ff>${rfmSlow}% kiệt sức <> trong <#e8a800ff>${rfmDur} giây<>."
$i18n.vi.rylais_crystal_scepter.option = "Sương Giá: Kĩ năng trúng kẻ địch sẽ gây <#d94c49ff>${rcsSlow}% kiệt sức<> trong <#e8a800ff>${rcsDur} giây<>."
$i18n.vi.radiant_rylais_crystal_scepter.option = "Sương Giá: Kĩ năng trúng kẻ địch sẽ gây <#d94c49ff>${rrcsSlow}% kiệt sức<> trong <#e8a800ff>${rrcsDur} giây<>."
...
$i18n.vi.blade_of_the_ruined_king.option = "Nanh vuốt sương mù: Đòn đánh thường gây thêm <#ff9028ff>sát thương vật lí<> tương ứng <#d94c49ff>(<$meleeIcon> ${borkMeleePct}% / <$rangedIcon> ${borkRangedPct}%) máu hiện tại của mục tiêu<>. Tối đa <#ff9028ff>${borkCap} sát thương vật lí<> lên lính và quái vật."
$i18n.vi.radiant_blade_of_the_ruined_king.option = "Nanh vuốt sương mù: Đòn đánh thường gây thêm <#ff9028ff>sát thương vật lí<> tương ứng <#d94c49ff>(<$meleeIcon> ${rborkMeleePct}% / <$rangedIcon> ${rborkRangedPct}%) máu hiện tại của mục tiêu<>. Tối đa <#ff9028ff>${rborkCap} sát thương vật lí<> lên lính và quái vật."
...
$i18n.vi.deaths_dance.option = "Phớt Lờ Đau Đớn: <#e8a800ff>${ddDelay}%<> sát thương nhận vào được tích trữ và gây lại cho bạn theo thời gian dưới dạng <#e3ab9dff>sát thương chuẩn<> (tối đa <#60e84dff>${ddBurnCap}%<> <$hpIcon> <#60e84dff>máu tối đa<> mỗi giây).`n`nCự Tuyệt: Tham gia hạ gục một tướng địch sẽ xóa lượng sát thương tích trữ còn lại và <$hpRegenIcon> <#60e84dff>hồi<> cho bạn <#60e84dff>${ddFlatHeal}<> + <#60e84dff>${ddHeal}%<> <#60e84dff>máu đã mất<>."
$i18n.vi.radiant_deaths_dance.option = "Phớt Lờ Đau Đớn: <#e8a800ff>${rddDelay}%<> sát thương nhận vào được tích trữ và gây lại cho bạn theo thời gian dưới dạng <#e3ab9dff>sát thương chuẩn<> (tối đa <#60e84dff>${rddBurnCap}%<> <$hpIcon> <#60e84dff>máu tối đa<> mỗi giây).`n`nCự Tuyệt: Tham gia hạ gục một tướng địch sẽ xóa lượng sát thương tích trữ còn lại và <$hpRegenIcon> <#60e84dff>hồi<> cho bạn <#60e84dff>${rddFlatHeal}<> + <#60e84dff>${rddHeal}%<> <#60e84dff>máu đã mất<>."
...
$i18n.vi.stormrazor.option = "Tích Điện: Di chuyển và đánh thường tạo ra điểm <#92dc7bff>Tích Điện<>, tối đa <#e8a800ff>${srStacks}<>.`n`nTia Sét: Khi <#92dc7bff>Tích Điện<> đầy, đòn đánh thường tiếp theo gây <#a974ffff>${srDmg} sát thương phép cộng thêm<> và tăng <#ffffffff>${srMs}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<> trong <#e8a800ff>${srDur} giây<>."
$i18n.vi.radiant_stormrazor.option = "Tích Điện: Di chuyển và đánh thường tạo ra điểm <#92dc7bff>Tích Điện<>, tối đa <#e8a800ff>${rsrStacks}<>.`n`nTia Sét: Khi <#92dc7bff>Tích Điện<> đầy, đòn đánh thường tiếp theo gây <#a974ffff>${rsrDmg} sát thương phép cộng thêm<> và tăng <#ffffffff>${rsrMs}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<> trong <#e8a800ff>${rsrDur} giây<>."
...
$eclVi = "Ánh Trăng Công Kích: Đòn đánh thường hoặc kĩ năng trúng tướng địch sẽ đánh dấu chúng trong <#e8a800ff>{0} giây<>, tối đa một lần mỗi lần thi triển. Đánh trúng tướng đã bị đánh dấu sẽ tiêu thụ dấu, gây <#ff9028ff>sát thương vật lí cộng thêm<> bằng <#d94c49ff>(<$meleeIcon> {1}% / <$rangedIcon> {6}%) máu tối đa của chúng<> và tạo cho bạn một <#cab944ff>khiên<> hấp thụ <#cab944ff>{2}<> + <#ff9028ff>{3}%<> <$adIcon> <#ff9028ff>SMCK<> của bạn trong <#e8a800ff>{4} giây<> (hồi chiêu <#e8a800ff>{5} giây<> mỗi mục tiêu)."
$i18n.vi.eclipse.option = $eclVi -f $eclDur, $eclMeleePct, $eclShield, $eclAdPct, $eclShDur, $eclCd, $eclRangedPct
$i18n.vi.radiant_eclipse.option = $eclVi -f $reclDur, $reclMeleePct, $reclShield, $reclAdPct, $reclShDur, $reclCd, $reclRangedPct
...
$rhTemplateVi = "Sát Thương Lan: Đòn đánh thường gây <#ff9028ff>(<$meleeIcon> {0}% / <$rangedIcon> {1}%)<> <$adIcon> <#ff9028ff>SMCK<> dưới dạng <#ff9028ff>sát thương vật lí<> lên kẻ địch trong phạm vi <#ff86c2ff>{2}<> <$rangeIcon> quanh mục tiêu."
$i18n.vi.ravenous_hydra.option = $rhTemplateVi -f $rhMeleePct, $rhRangedPct, $rhRange
$i18n.vi.radiant_ravenous_hydra.option = $rhTemplateVi -f $rrhMeleePct, $rrhRangedPct, $rrhRange
...
$bsTemplateVi = "Kiếm Phép: Kĩ năng trúng tướng địch khiến đòn đánh thường tiếp theo của bạn gây <#a974ffff>{0}<> - <#a974ffff>{1}<> (dựa theo <$levelIcon> <#d8c9b3ff>cấp độ<>) dưới dạng <#a974ffff>sát thương phép cộng thêm<> (hồi chiêu <#e8a800ff>{2} giây<>). Nếu mục tiêu là tướng, tăng <#d94c49ff>sát thương chúng phải nhận<> thêm <#d94c49ff>(<$meleeIcon> {3}% / <$rangedIcon> {5}%)<> trong <#e8a800ff>{4} giây<>."
$i18n.vi.bloodsong.option = $bsTemplateVi -f $bsMin, $bsMax, $bsCd, $bsMeleeAmp, $bsDur, $bsRangedAmp
$i18n.vi.radiant_bloodsong.option = $bsTemplateVi -f $rbsMin, $rbsMax, $rbsCd, $rbsMeleeAmp, $rbsDur, $rbsRangedAmp
...
$bbVi = "$lethVi`n`nPhá Hoại: Tham gia hạ gục một tướng địch sẽ trao <#92dc7bff>Phá Hoại<> trong <#e8a800ff>{3} giây<>, tăng cường đòn đánh thường tiếp theo của bạn lên trụ để gây <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> <$adIcon> <#ff9028ff>SMCK<> dưới dạng <#e3ab9dff>sát thương chuẩn cộng thêm<>."
...
$spfVi = "$lethVi`n`nKẻ Cắt Khiên: Đòn đánh thường hoặc kĩ năng trúng tướng địch đang có lá chắn sẽ gây thêm <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> <$adIcon> <#ff9028ff>SMCK<> dưới dạng <#ff9028ff>sát thương vật lí cộng thêm<>."
...
$oppVi = "$lethVi`n`nChuẩn Bị: Sau khi ở ngoài giao tranh với tướng địch trong <#e8a800ff>{1} giây<>, nhận thêm <#ffdd8eff>{2} <$armorPenIcon> Xuyên Giáp Trắng<>. Hiệu ứng này duy trì thêm <#e8a800ff>{3} giây<> sau khi gây sát thương lên tướng địch."
...
$lockVi = "Tận Hiến: <#d94c49ff>Máu rơi xuống dưới {5}%<> sẽ tạo cho bạn và mọi tướng đồng minh lân cận một <#cab944ff>khiên<> <#cab944ff>hấp thụ<> <#cab944ff>{6}<> - <#cab944ff>{7}<> (dựa theo <$levelIcon> <#d8c9b3ff>cấp độ<>) <#cab944ff>sát thương<> trong <#e8a800ff>{8} giây<> (<#e8a800ff>{9} giây<> hồi chiêu).`n`nQuân Đoàn: Cấp <#ffdd8eff>{0} <$armorIcon> giáp<>, <#88ccffff>{1} <$mrIcon> kháng phép<> và <#60e84dff>{2} <$hpRegenIcon> hồi máu mỗi giây<> cho mọi đồng minh trong phạm vi <#ff86c2ff>{4}<> <$rangeIcon> . Lính nhận <#e8a800ff>{3}%<> giá trị này."
...
$dmpVi = "Kẻ Đắm Tàu: Khi di chuyển, tích <#e8a800ff>{0} cộng dồn<> <#92dc7bff>Đà<> mỗi giây, tối đa <#e8a800ff>{1}<>. Mỗi cộng dồn tăng <#ffffffff>{2}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<>. Đòn đánh thường tiêu hết <#92dc7bff>Đà<> còn lại để gây <#ff9028ff>{3} - {4} sát thương vật lí cộng thêm<>, tăng theo số cộng dồn đã tiêu."
...
$i18n.'zh-hans'.stormsurge.option = "风暴掠袭：在 <#e8a800ff>${surgeWin}秒<> 内对敌方英雄造成相当于其 <#60e84dff>${surgePct}%<> <$hpIcon> <#60e84dff>最大生命值<> 的伤害时，对其施加风啸（每个目标冷却 <#e8a800ff>${surgeCd}秒<>）。`n`n风啸：<#e8a800ff>${surgeDelay}秒<> 后以闪电打击目标，造成 <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> <$apIcon> <#a974ffff>法术强度<> 的<#a974ffff>魔法伤害<>。"
$i18n.'zh-hans'.radiant_stormsurge.option = "风暴掠袭：在 <#e8a800ff>${rsurgeWin}秒<> 内对敌方英雄造成相当于其 <#60e84dff>${rsurgePct}%<> <$hpIcon> <#60e84dff>最大生命值<> 的伤害时，对其施加风啸（每个目标冷却 <#e8a800ff>${rsurgeCd}秒<>）。`n`n风啸：<#e8a800ff>${rsurgeDelay}秒<> 后以闪电打击目标，造成 <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> <$apIcon> <#a974ffff>法术强度<> 的<#a974ffff>魔法伤害<>。"
$i18n.'zh-hans'.fated_ashes.option = "激燃：技能命中敌人时点燃目标，在 <#e8a800ff>${ashDur} 秒<>内造成 <#a974ffff>${ashFlat} 点魔法伤害<>。该效果对小兵和野怪的<#e8a800ff>效果为 ${ashPct}%<>。"
$i18n.'zh-hans'.hextech_alternator.option = "转速加快：对敌方英雄造成伤害时，额外造成 <#a974ffff>${altFlat} 点魔法伤害<>（冷却 <#e8a800ff>${altCd}秒<>）。"
$i18n.'zh-hans'.protectors_vow.option = "敬畏：获得 <#60e84dff>${pvFlat}<> + 你的 <$armorIcon> <#ffdd8eff>护甲<> 的 <#ffdd8eff>${pvArmorPct}%<> 作为<$hpIcon> <#60e84dff>最大生命值<>。"
...
$i18n.'zh-hans'.shadowflame.option = "余烬绽放：你的<#a974ffff>魔法伤害<>和<#e3ab9dff>真实伤害<>对<#d94c49ff>最大生命值低于${sfThreshold}%<>的敌人<#e8a800ff>提高20%<>。"
$i18n.'zh-hans'.radiant_shadowflame.option = "余烬绽放：你的<#a974ffff>魔法伤害<>和<#e3ab9dff>真实伤害<>对<#d94c49ff>最大生命值低于${rsfThreshold}%<>的敌人<#e8a800ff>提高20%<>。"
$i18n.'zh-hans'.yun_tal_wildarrows.option = "熟能生巧：普通攻击永久获得 <#e8a800ff>${ytCrit}%<> <$critIcon> <#e8a800ff>暴击几率<>，最多 <#e8a800ff>${ytMaxCrit}%<>。`n`n疾风骤雨：每隔 <#e8a800ff>${ytCd}秒<>，你的下一次普通攻击获得 <#ceff99ff>${ytFlurryAS}%<> <$asIcon> <#ceff99ff>攻击速度<>，持续 <#e8a800ff>${ytDur}秒<>。"
$i18n.'zh-hans'.radiant_yun_tal_wildarrows.option = "熟能生巧：普通攻击永久获得 <#e8a800ff>${rytCrit}%<> <$critIcon> <#e8a800ff>暴击几率<>，最多 <#e8a800ff>${rytMaxCrit}%<>。`n`n疾风骤雨：每隔 <#e8a800ff>${rytCd}秒<>，你的下一次普通攻击获得 <#ceff99ff>${rytFlurryAS}%<> <$asIcon> <#ceff99ff>攻击速度<>，持续 <#e8a800ff>${rytDur}秒<>。"
$i18n.'zh-hans'.phage.option = "狂怒：普通攻击敌方英雄时，获得 <#ffffffff>${phageMs}%<> <$speedIcon> <#ffffffff>移动速度<>，持续 <#e8a800ff>${phageDur}秒<>。"
...
$i18n.'zh-hans'.rylais_crystal_scepter.option = "凛霜：技能命中敌人时，对其施加 <#d94c49ff>${rcsSlow}%减速<>，持续 <#e8a800ff>${rcsDur}秒<>。"
$i18n.'zh-hans'.radiant_rylais_crystal_scepter.option = "凛霜：技能命中敌人时，对其施加 <#d94c49ff>${rrcsSlow}%减速<>，持续 <#e8a800ff>${rrcsDur}秒<>。"
...
$i18n.'zh-hans'.blade_of_the_ruined_king.option = "雾之锋：普通攻击造成相当于目标<#d94c49ff>当前生命值(<$meleeIcon> ${borkMeleePct}% / <$rangedIcon> ${borkRangedPct}%)<>的<#ff9028ff>额外物理伤害<>。对小兵和野怪最多造成 <#ff9028ff>${borkCap}点物理伤害<>。"
$i18n.'zh-hans'.radiant_blade_of_the_ruined_king.option = "雾之锋：普通攻击造成相当于目标<#d94c49ff>当前生命值(<$meleeIcon> ${rborkMeleePct}% / <$rangedIcon> ${rborkRangedPct}%)<>的<#ff9028ff>额外物理伤害<>。对小兵和野怪最多造成 <#ff9028ff>${rborkCap}点物理伤害<>。"
...
$i18n.'zh-hans'.deaths_dance.option = "无视痛苦：受到伤害的 <#e8a800ff>${ddDelay}%<> 会被储存，并随时间作为 <#e3ab9dff>额外真实伤害<> 返还给你（每秒最多为你的 <$hpIcon> <#60e84dff>最大生命值<>的 <#60e84dff>${ddBurnCap}%<>）。`n`n拒止：参与击杀敌方英雄会清除剩余的储存伤害，并为你<$hpRegenIcon> <#60e84dff>回复<> <#60e84dff>${ddFlatHeal}<> + <#60e84dff>${ddHeal}%<> 的<#60e84dff>已损失生命值<>。"
$i18n.'zh-hans'.radiant_deaths_dance.option = "无视痛苦：受到伤害的 <#e8a800ff>${rddDelay}%<> 会被储存，并随时间作为 <#e3ab9dff>额外真实伤害<> 返还给你（每秒最多为你的 <$hpIcon> <#60e84dff>最大生命值<>的 <#60e84dff>${rddBurnCap}%<>）。`n`n拒止：参与击杀敌方英雄会清除剩余的储存伤害，并为你<$hpRegenIcon> <#60e84dff>回复<> <#60e84dff>${rddFlatHeal}<> + <#60e84dff>${rddHeal}%<> 的<#60e84dff>已损失生命值<>。"
...
$i18n.'zh-hans'.stormrazor.option = "蓄能：移动和普通攻击会产生<#92dc7bff>蓄能<>层数，最多 <#e8a800ff>${srStacks}<> 层。`n`n雷霆：<#92dc7bff>蓄能<>充满时，你的下一次普通攻击额外造成 <#a974ffff>${srDmg} 点魔法伤害<>，并使你获得 <#ffffffff>${srMs}%<> <$speedIcon> <#ffffffff>移动速度<>，持续 <#e8a800ff>${srDur}秒<>。"
$i18n.'zh-hans'.radiant_stormrazor.option = "蓄能：移动和普通攻击会产生<#92dc7bff>蓄能<>层数，最多 <#e8a800ff>${rsrStacks}<> 层。`n`n雷霆：<#92dc7bff>蓄能<>充满时，你的下一次普通攻击额外造成 <#a974ffff>${rsrDmg} 点魔法伤害<>，并使你获得 <#ffffffff>${rsrMs}%<> <$speedIcon> <#ffffffff>移动速度<>，持续 <#e8a800ff>${rsrDur}秒<>。"
...
$eclZh = "永升之月：普通攻击或技能命中敌方英雄时会标记其 <#e8a800ff>{0}秒<>，每次施放最多触发一次。命中已被标记的英雄会消耗该标记，造成相当于<#d94c49ff>其(<$meleeIcon> {1}% / <$rangedIcon> {6}%)最大生命值<>的<#ff9028ff>额外物理伤害<>，并为你获得可吸收 <#cab944ff>{2}<> + <$adIcon> <#ff9028ff>攻击力的 {3}%<> 的<#cab944ff>护盾<>，持续 <#e8a800ff>{4}秒<>（每个目标冷却 <#e8a800ff>{5}秒<>）。"
$i18n.'zh-hans'.eclipse.option = $eclZh -f $eclDur, $eclMeleePct, $eclShield, $eclAdPct, $eclShDur, $eclCd, $eclRangedPct
$i18n.'zh-hans'.radiant_eclipse.option = $eclZh -f $reclDur, $reclMeleePct, $reclShield, $reclAdPct, $reclShDur, $reclCd, $reclRangedPct
$i18n.'zh-hans'.echoes_of_helia.option = "灵魂虹吸：将你造成和受到伤害的 <#e8a800ff>${eohConversion}%<> 储存为 <#92dc7bff>灵魂充能<>，最多 <$levelIcon> <#d8c9b3ff>${eohMinCap}<> - <#d8c9b3ff>${eohMaxCap}<>（随<#d8c9b3ff>等级<>提升）。为友方英雄（自己除外）回复生命、提供护盾或增益时，消耗全部 <#92dc7bff>灵魂充能<>，为其<$hpRegenIcon> <#60e84dff>回复<>等量的生命值。"
$i18n.'zh-hans'.radiant_echoes_of_helia.option = "灵魂虹吸：将你造成和受到伤害的 <#e8a800ff>${reohConversion}%<> 储存为 <#92dc7bff>灵魂充能<>，最多 <$levelIcon> <#d8c9b3ff>${reohMinCap}<> - <#d8c9b3ff>${reohMaxCap}<>（随<#d8c9b3ff>等级<>提升）。为友方英雄（自己除外）回复生命、提供护盾或增益时，消耗全部 <#92dc7bff>灵魂充能<>，为其<$hpRegenIcon> <#60e84dff>回复<>等量的生命值。"
...
$rhTemplateZh = "顺劈：普通攻击会对目标周围 <#ff86c2ff>{2} <$rangeIcon> 射程<>内的敌人造成相当于你 <$adIcon> <#ff9028ff>攻击力<>的 <#ff9028ff>(<$meleeIcon> {0}% / <$rangedIcon> {1}%)<> 的<#ff9028ff>物理伤害<>。"
$i18n.'zh-hans'.ravenous_hydra.option = $rhTemplateZh -f $rhMeleePct, $rhRangedPct, $rhRange
$i18n.'zh-hans'.radiant_ravenous_hydra.option = $rhTemplateZh -f $rrhMeleePct, $rrhRangedPct, $rrhRange
...
$bsTemplateZh = "咒刃：技能命中敌方英雄后，你的下一次普通攻击会造成 <#a974ffff>{0}<> - <#a974ffff>{1}<>（基于<$levelIcon> <#d8c9b3ff>等级<>）的<#a974ffff>额外魔法伤害<>（冷却 <#e8a800ff>{2}秒<>）。如果目标是英雄，使其<#d94c49ff>受到的伤害<>提高 <#d94c49ff>(<$meleeIcon> {3}% / <$rangedIcon> {5}%)<>，持续 <#e8a800ff>{4}秒<>。"
$i18n.'zh-hans'.bloodsong.option = $bsTemplateZh -f $bsMin, $bsMax, $bsCd, $bsMeleeAmp, $bsDur, $bsRangedAmp
$i18n.'zh-hans'.radiant_bloodsong.option = $bsTemplateZh -f $rbsMin, $rbsMax, $rbsCd, $rbsMeleeAmp, $rbsDur, $rbsRangedAmp
...
$bbZh = "$lethZh`n`n破坏：参与击杀敌方英雄时，获得 <#92dc7bff>破坏<> 效果，持续 <#e8a800ff>{3}秒<>，强化你对防御塔的下一次普通攻击，造成 <#ff9028ff>{1}<> + <$adIcon> <#ff9028ff>攻击力<>的 <#ff9028ff>{2}%<> 作为<#e3ab9dff>额外真实伤害<>。"
...
$spfZh = "$lethZh`n`n破盾者：普通攻击或技能命中拥有护盾的敌方英雄时，额外造成 <#ff9028ff>{1}<> + <$adIcon> <#ff9028ff>攻击力<>的 <#ff9028ff>{2}%<> 作为<#ff9028ff>额外物理伤害<>。"
...
$oppZh = "$lethZh`n`n伺机待发：脱离与敌方英雄的战斗达 <#e8a800ff>{1}秒<> 后，获得 <#ffdd8eff>{2} 点<$armorPenIcon> 穿甲<>。该加成在对敌方英雄造成伤害后保留 <#e8a800ff>{3}秒<>。"
...
$lockZh = "虔诚：<#d94c49ff>生命值降至{5}%以下<>时，为你和附近的友方英雄提供一个持续 <#e8a800ff>{8}秒<> 的<#cab944ff>护盾<>，<#cab944ff>吸收<> <#cab944ff>{6}<> - <#cab944ff>{7}<>（基于<$levelIcon> <#d8c9b3ff>等级<>）<#cab944ff>伤害<>（冷却时间{9}秒）。`n`n军团：为 <#ff86c2ff>{4} <$rangeIcon> 射程<>范围内的所有友军提供 <#ffdd8eff>{0} 点<$armorIcon> 护甲<>、<#88ccffff>{1} 点<$mrIcon> 魔法抗性<>和 <#60e84dff>{2} 点<$hpRegenIcon> 生命回复<>。小兵获得该数值的 <#e8a800ff>{3}%<>。"
...
$dmpZh = "碎船者：移动时每秒获得 <#e8a800ff>{0} 层<><#92dc7bff>动量<>，最多 <#e8a800ff>{1}<> 层。每层提供 <#ffffffff>{2}%<> <$speedIcon> <#ffffffff>额外移动速度<>。普通攻击消耗所有剩余<#92dc7bff>动量<>，造成 <#ff9028ff>{3} - {4} 点额外物理伤害<>，随消耗的层数提升。"
...
$i18n.'pt-BR'.stormsurge.option = "Invasor da Tempestade: Causar dano equivalente a <#60e84dff>${surgePct}%<> da <$hpIcon> <#60e84dff>Vida máxima<> de um campeão inimigo dentro de <#e8a800ff>${surgeWin} segundos<> aplica Dilúvio a ele (recarga de <#e8a800ff>${surgeCd} segundos<> por alvo).`n`nDilúvio: Depois de <#e8a800ff>${surgeDelay} segundos<>, atinge o alvo com um raio, causando <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> de <$apIcon> <#a974ffff>Poder de Habilidade<> como <#a974ffff>dano mágico<>."
$i18n.'pt-BR'.radiant_stormsurge.option = "Invasor da Tempestade: Causar dano equivalente a <#60e84dff>${rsurgePct}%<> da <$hpIcon> <#60e84dff>Vida máxima<> de um campeão inimigo dentro de <#e8a800ff>${rsurgeWin} segundos<> aplica Dilúvio a ele (recarga de <#e8a800ff>${rsurgeCd} segundos<> por alvo).`n`nDilúvio: Depois de <#e8a800ff>${rsurgeDelay} segundos<>, atinge o alvo com um raio, causando <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> de <$apIcon> <#a974ffff>Poder de Habilidade<> como <#a974ffff>dano mágico<>."
$i18n.'pt-BR'.fated_ashes.option = "Inflamar: Acertar uma Habilidade em um inimigo o queima, causando <#a974ffff>${ashFlat} de dano mágico<> ao longo de <#e8a800ff>${ashDur} segundos<>. Este efeito é <#e8a800ff>${ashPct}% efetivo<> contra lacaios e monstros."
$i18n.'pt-BR'.hextech_alternator.option = "Entusiasmo: Causar dano a um campeão inimigo causa <#a974ffff>${altFlat} de dano mágico bônus<> (recarga de <#e8a800ff>${altCd} segundos<>)."
$i18n.'pt-BR'.protectors_vow.option = "Temor: Ganha <$hpIcon> <#60e84dff>Vida Máxima<> igual a <#60e84dff>${pvFlat}<> + <#ffdd8eff>${pvArmorPct}%<> da sua <$armorIcon> <#ffdd8eff>Armadura<>."
...
$i18n.'pt-BR'.shadowflame.option = "Floregris: Seu <#a974ffff>dano mágico<> e <#e3ab9dff>dano verdadeiro<> são <#e8a800ff>20% mais fortes<> contra inimigos <#d94c49ff>abaixo de ${sfThreshold}% da Vida Máxima<>."
$i18n.'pt-BR'.radiant_shadowflame.option = "Floregris: Seu <#a974ffff>dano mágico<> e <#e3ab9dff>dano verdadeiro<> são <#e8a800ff>20% mais fortes<> contra inimigos <#d94c49ff>abaixo de ${rsfThreshold}% da Vida Máxima<>."
$i18n.'pt-BR'.yun_tal_wildarrows.option = "Praticar e Matar: Ataques básicos concedem <#e8a800ff>${ytCrit}%<> de <$critIcon> <#e8a800ff>Chance de Acerto Crítico<> permanentemente, até <#e8a800ff>${ytMaxCrit}%<>.`n`nAgitação: A cada <#e8a800ff>${ytCd} segundos<>, seu próximo ataque básico concede <#ceff99ff>${ytFlurryAS}%<> de <$asIcon> <#ceff99ff>Velocidade de Ataque<> por <#e8a800ff>${ytDur} segundos<>."
$i18n.'pt-BR'.radiant_yun_tal_wildarrows.option = "Praticar e Matar: Ataques básicos concedem <#e8a800ff>${rytCrit}%<> de <$critIcon> <#e8a800ff>Chance de Acerto Crítico<> permanentemente, até <#e8a800ff>${rytMaxCrit}%<>.`n`nAgitação: A cada <#e8a800ff>${rytCd} segundos<>, seu próximo ataque básico concede <#ceff99ff>${rytFlurryAS}%<> de <$asIcon> <#ceff99ff>Velocidade de Ataque<> por <#e8a800ff>${rytDur} segundos<>."
$i18n.'pt-BR'.phage.option = "Fúria: Ataques contra campeões inimigos concedem <#ffffffff>${phageMs}%<> de <$speedIcon> <#ffffffff>Velocidade de Movimento<> por <#e8a800ff>${phageDur} segundos<>."
...
$i18n.'pt-BR'.rylais_crystal_scepter.option = "Gélido: Acertar uma Habilidade em um inimigo aplica <#d94c49ff>${rcsSlow}% de lentidão<> por <#e8a800ff>${rcsDur} segundos<>."
$i18n.'pt-BR'.radiant_rylais_crystal_scepter.option = "Gélido: Acertar uma Habilidade em um inimigo aplica <#d94c49ff>${rrcsSlow}% de lentidão<> por <#e8a800ff>${rrcsDur} segundos<>."
...
$i18n.'pt-BR'.blade_of_the_ruined_king.option = "Gume da Névoa: Ataques causam <#d94c49ff>(<$meleeIcon> ${borkMeleePct}% / <$rangedIcon> ${borkRangedPct}%)<> da <#d94c49ff>Vida Atual do alvo<> como <#ff9028ff>dano físico<> adicional. (Máximo de <#ff9028ff>${borkCap}<> contra tropas e monstros.)"
$i18n.'pt-BR'.radiant_blade_of_the_ruined_king.option = "Gume da Névoa: Ataques causam <#d94c49ff>(<$meleeIcon> ${rborkMeleePct}% / <$rangedIcon> ${rborkRangedPct}%)<> da <#d94c49ff>Vida Atual do alvo<> como <#ff9028ff>dano físico<> adicional. (Máximo de <#ff9028ff>${rborkCap}<> contra tropas e monstros.)"
...
$i18n.'pt-BR'.radiant_heartsteel.option = "Coração Forjado: A cada <#e8a800ff>${rhsCd} segundos<>, seu próximo ataque básico causa <#ff9028ff>dano físico<> adicional igual a <#ff9028ff>${rhsFlat}<> + <#60e84dff>${rhsHpPct}%<> da sua <$hpIcon> <#60e84dff>Vida Máxima<>, concedendo permanentemente <#60e84dff>${rhsBonusHpPct}%<> desse dano como <#60e84dff>vida bônus<>."
...
$eclPt = "Lua Sempre Crescente: Acertar um ataque básico ou uma Habilidade em um campeão inimigo o marca por <#e8a800ff>{0} segundos<>, no máximo uma vez por instância de conjuração. Atingir um campeão marcado consome a marca para causar <#ff9028ff>dano físico bônus<> equivalente a <#d94c49ff>(<$meleeIcon> {1}% / <$rangedIcon> {6}%) da Vida Máxima dele<> e conceder a você um <#cab944ff>escudo<> que absorve <#cab944ff>{2}<> + <#ff9028ff>{3}%<> do seu <$adIcon> <#ff9028ff>Dano de Ataque<> por <#e8a800ff>{4} segundos<> (recarga de <#e8a800ff>{5} segundos<> por alvo)."
$i18n.'pt-BR'.eclipse.option = $eclPt -f $eclDur, $eclMeleePct, $eclShield, $eclAdPct, $eclShDur, $eclCd, $eclRangedPct
$i18n.'pt-BR'.radiant_eclipse.option = $eclPt -f $reclDur, $reclMeleePct, $reclShield, $reclAdPct, $reclShDur, $reclCd, $reclRangedPct
...
$rhTemplatePt = "Fenda: Ataques básicos causam <#ff9028ff>(<$meleeIcon> {0}% / <$rangedIcon> {1}%)<> do seu <$adIcon> <#ff9028ff>Dano de Ataque<> como <#ff9028ff>dano físico<> a inimigos dentro de <#ff86c2ff>{2} <$rangeIcon> alcance<> do alvo."
$i18n.'pt-BR'.ravenous_hydra.option = $rhTemplatePt -f $rhMeleePct, $rhRangedPct, $rhRange
$i18n.'pt-BR'.radiant_ravenous_hydra.option = $rhTemplatePt -f $rrhMeleePct, $rrhRangedPct, $rrhRange
...
$bsTemplatePt = "Lâmina Arcana: Acertar uma Habilidade em um campeão inimigo faz seu próximo ataque básico causar <#a974ffff>{0}<> - <#a974ffff>{1}<> (com base no <$levelIcon> <#d8c9b3ff>nível<>) como <#a974ffff>dano mágico bônus<> (recarga de <#e8a800ff>{2} segundos<>). Se o alvo for um campeão, aumenta o <#d94c49ff>dano que ele recebe<> em <#d94c49ff>(<$meleeIcon> {3}% / <$rangedIcon> {5}%)<> por <#e8a800ff>{4} segundos<>."
$i18n.'pt-BR'.bloodsong.option = $bsTemplatePt -f $bsMin, $bsMax, $bsCd, $bsMeleeAmp, $bsDur, $bsRangedAmp
$i18n.'pt-BR'.radiant_bloodsong.option = $bsTemplatePt -f $rbsMin, $rbsMax, $rbsCd, $rbsMeleeAmp, $rbsDur, $rbsRangedAmp
...
$bbPt = "$lethPt`n`nSabotagem: Ao participar do abate de um campeão inimigo, concede <#92dc7bff>Sabotagem<> por <#e8a800ff>{3} segundos<>, potencializando seu próximo ataque básico contra uma torre para causar <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> do seu <$adIcon> <#ff9028ff>Dano de Ataque<> como <#e3ab9dff>dano verdadeiro bônus<>."
...
$spfPt = "$lethPt`n`nCeifador de Escudos: Acertar um ataque básico ou uma Habilidade em um campeão inimigo com escudo causa <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> do seu <$adIcon> <#ff9028ff>Dano de Ataque<> como <#ff9028ff>dano físico bônus<>."
...
$oppPt = "$lethPt`n`nPreparação: Após ficar fora de combate com campeões inimigos por <#e8a800ff>{1} segundos<>, ganha <#ffdd8eff>{2} de <$armorPenIcon> Letalidade<>. Este bônus permanece por <#e8a800ff>{3} segundos<> após causar dano a um campeão inimigo."
...
$lockPt = "Devoção: Ao cair <#d94c49ff>abaixo de {5}% de Vida<>, concede a você e a todos os campeões aliados próximos um <#cab944ff>escudo<> que <#cab944ff>absorve<> <#cab944ff>{6}<> - <#cab944ff>{7}<> (com base no <$levelIcon> <#d8c9b3ff>nível<>) de <#cab944ff>dano<> por <#e8a800ff>{8} segundos<> (recarga de <#e8a800ff>{9} segundos<>).`n`nLegião: Concede <#ffdd8eff>{0} de <$armorIcon> Armadura<>, <#88ccffff>{1} de <$mrIcon> Resistência Mágica<> e <#60e84dff>{2} de <$hpRegenIcon> Regeneração de Vida<> a todos os aliados dentro de <#ff86c2ff>{4} <$rangeIcon> alcance<>. Tropas recebem <#e8a800ff>{3}%<> deste valor."
...
$i18n.ru.stormsurge.option = "Штормовой налет: Если вы наносите вражескому чемпиону урон в размере <#60e84dff>${surgePct}%<> от его <$hpIcon> <#60e84dff>максимального запаса здоровья<> в течение <#e8a800ff>${surgeWin} секунд<>, к нему применяется Порыв (перезарядка <#e8a800ff>${surgeCd} секунд<> на каждую цель).`n`nПорыв: Через <#e8a800ff>${surgeDelay} секунды<> молния поражает цель, нанося <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> <$apIcon> <#a974ffff>Силы Умений<> как <#a974ffff>магический урон<>."
$i18n.ru.radiant_stormsurge.option = "Штормовой налет: Если вы наносите вражескому чемпиону урон в размере <#60e84dff>${rsurgePct}%<> от его <$hpIcon> <#60e84dff>максимального запаса здоровья<> в течение <#e8a800ff>${rsurgeWin} секунд<>, к нему применяется Порыв (перезарядка <#e8a800ff>${rsurgeCd} секунд<> на каждую цель).`n`nПорыв: Через <#e8a800ff>${rsurgeDelay} секунды<> молния поражает цель, нанося <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> <$apIcon> <#a974ffff>Силы Умений<> как <#a974ffff>магический урон<>."
$i18n.ru.fated_ashes.option = "Разгорание: Попадание умением по противнику поджигает его, нанося <#a974ffff>${ashFlat} магического урона<> в течение <#e8a800ff>${ashDur} сек.<> Против миньонов и монстров эффект <#e8a800ff>${ashPct}%<>."
$i18n.ru.hextech_alternator.option = "На взводе: Нанесение урона вражескому чемпиону наносит <#a974ffff>${altFlat} дополнительного магического урона<> (перезарядка <#e8a800ff>${altCd} секунд<>)."
$i18n.ru.protectors_vow.option = "Трепет: Даёт <$hpIcon> <#60e84dff>максимальное здоровье<> равное <#60e84dff>${pvFlat}<> + <#ffdd8eff>${pvArmorPct}%<> вашей <$armorIcon> <#ffdd8eff>брони<>."
...
$i18n.ru.shadowflame.option = "Огненный цветок: Ваш <#a974ffff>магический урон<> и <#e3ab9dff>чистый урон<> <#e8a800ff>на 20% сильнее<> против врагов, чьё <#d94c49ff>здоровье ниже ${sfThreshold}% от максимального<>."
$i18n.ru.radiant_shadowflame.option = "Огненный цветок: Ваш <#a974ffff>магический урон<> и <#e3ab9dff>чистый урон<> <#e8a800ff>на 20% сильнее<> против врагов, чьё <#d94c49ff>здоровье ниже ${rsfThreshold}% от максимального<>."
$i18n.ru.yun_tal_wildarrows.option = "Убийственная практика: Базовые атаки навсегда дают <#e8a800ff>${ytCrit}%<> <$critIcon> <#e8a800ff>шанса критического удара<>, вплоть до <#e8a800ff>${ytMaxCrit}%<>.`n`nШквал: Каждые <#e8a800ff>${ytCd} секунд<> ваша следующая базовая атака даёт <#ceff99ff>${ytFlurryAS}%<> <$asIcon> <#ceff99ff>скорости атаки<> на <#e8a800ff>${ytDur} секунд<>."
$i18n.ru.radiant_yun_tal_wildarrows.option = "Убийственная практика: Базовые атаки навсегда дают <#e8a800ff>${rytCrit}%<> <$critIcon> <#e8a800ff>шанса критического удара<>, вплоть до <#e8a800ff>${rytMaxCrit}%<>.`n`nШквал: Каждые <#e8a800ff>${rytCd} секунд<> ваша следующая базовая атака даёт <#ceff99ff>${rytFlurryAS}%<> <$asIcon> <#ceff99ff>скорости атаки<> на <#e8a800ff>${rytDur} секунд<>."
$i18n.ru.phage.option = "Ярость: Базовые атаки по вражеским чемпионам дают <#ffffffff>${phageMs}%<> <$speedIcon> <#ffffffff>скорости передвижения<> на <#e8a800ff>${phageDur} секунды<>."
...
$i18n.ru.frozen_mallet.option = "Обледенение: Базовые атаки накладывают <#d94c49ff>замедление на ${fmSlow}%<> на <#e8a800ff>${fmDur} секунды<>."
$i18n.ru.radiant_frozen_mallet.option = "Обледенение: Базовые атаки наносят <#ff9028ff>дополнительный физический урон<> равный <#ff9028ff>${rfmFlat}<> + <#60e84dff>${rfmHpPct}%<> от вашего <$hpIcon> <#60e84dff>максимального здоровья<> и накладывают <#d94c49ff>замедление на ${rfmSlow}%<> на <#e8a800ff>${rfmDur} секунды<>."
$i18n.ru.rylais_crystal_scepter.option = "Иней: Попадание умением по противнику замедляет его на <#d94c49ff>${rcsSlow}%<> на <#e8a800ff>${rcsDur} секунды<>."
$i18n.ru.radiant_rylais_crystal_scepter.option = "Иней: Попадание умением по противнику замедляет его на <#d94c49ff>${rrcsSlow}%<> на <#e8a800ff>${rrcsDur} секунды<>."
...
$i18n.ru.radiant_wits_end.option = "Боевой запал: Базовые атаки наносят <#a974ffff>${rweDmg} дополнительного магического урона<>."
...
$i18n.ru.blade_of_the_ruined_king.option = "Край тумана: Базовые атаки наносят <#ff9028ff>дополнительный физический урон<> равный <#d94c49ff>(<$meleeIcon> ${borkMeleePct}% / <$rangedIcon> ${borkRangedPct}%) текущего здоровья цели<>. Наносит максимум <#ff9028ff>${borkCap} физического урона<> по миньонам и монстрам."
$i18n.ru.radiant_blade_of_the_ruined_king.option = "Край тумана: Базовые атаки наносят <#ff9028ff>дополнительный физический урон<> равный <#d94c49ff>(<$meleeIcon> ${rborkMeleePct}% / <$rangedIcon> ${rborkRangedPct}%) текущего здоровья цели<>. Наносит максимум <#ff9028ff>${rborkCap} физического урона<> по миньонам и монстрам."
...
$i18n.ru.stormrazor.option = "Заряд: Движение и базовые атаки генерируют заряды <#92dc7bff>Энергии<>, до <#e8a800ff>${srStacks}<>.`n`nМолния: При полном <#92dc7bff>Заряде<>, ваша следующая базовая атака наносит <#a974ffff>${srDmg} дополнительного магического урона<> и даёт <#ffffffff>${srMs}%<> <$speedIcon> <#ffffffff>скорости передвижения<> на <#e8a800ff>${srDur} секунды<>."
$i18n.ru.radiant_stormrazor.option = "Заряд: Движение и базовые атаки генерируют заряды <#92dc7bff>Энергии<>, до <#e8a800ff>${rsrStacks}<>.`n`nМолния: При полном <#92dc7bff>Заряде<>, ваша следующая базовая атака наносит <#a974ffff>${rsrDmg} дополнительного магического урона<> и даёт <#ffffffff>${rsrMs}%<> <$speedIcon> <#ffffffff>скорости передвижения<> на <#e8a800ff>${rsrDur} секунды<>."
...
$eclRu = "Восходящая луна: Попадание базовой атакой или умением по вражескому чемпиону отмечает его на <#e8a800ff>{0} секунды<>, не более одного раза за применение. Попадание по отмеченному чемпиону поглощает метку, нанося <#ff9028ff>дополнительный физический урон<> в размере <#d94c49ff>(<$meleeIcon> {1}% / <$rangedIcon> {6}%) его максимального здоровья<> и давая вам <#cab944ff>щит<>, поглощающий <#cab944ff>{2}<> + <#ff9028ff>{3}%<> вашей <$adIcon> <#ff9028ff>силы атаки<> на <#e8a800ff>{4} секунды<> (перезарядка <#e8a800ff>{5} секунд<> на каждую цель)."
$i18n.ru.eclipse.option = $eclRu -f $eclDur, $eclMeleePct, $eclShield, $eclAdPct, $eclShDur, $eclCd, $eclRangedPct
$i18n.ru.radiant_eclipse.option = $eclRu -f $reclDur, $reclMeleePct, $reclShield, $reclAdPct, $reclShDur, $reclCd, $reclRangedPct
...
$rhTemplateRu = "Рассечение: Базовые атаки наносят <#ff9028ff>(<$meleeIcon> {0}% / <$rangedIcon> {1}%)<> вашей <$adIcon> <#ff9028ff>силы атаки<> в виде <#ff9028ff>физического урона<> врагам в пределах <#ff86c2ff>{2} <$rangeIcon> дальности<> от цели."
$i18n.ru.ravenous_hydra.option = $rhTemplateRu -f $rhMeleePct, $rhRangedPct, $rhRange
$i18n.ru.radiant_ravenous_hydra.option = $rhTemplateRu -f $rrhMeleePct, $rrhRangedPct, $rrhRange
...
$bsTemplateRu = "Чародейский клинок: Попадание умением по вражескому чемпиону заставляет вашу следующую базовую атаку нанести <#a974ffff>{0}<> - <#a974ffff>{1}<> (в зависимости от <$levelIcon> <#d8c9b3ff>уровня<>) в виде <#a974ffff>дополнительного магического урона<> (перезарядка <#e8a800ff>{2} секунд<>). Если цель — чемпион, увеличивает <#d94c49ff>получаемый ею урон<> на <#d94c49ff>(<$meleeIcon> {3}% / <$rangedIcon> {5}%)<> на <#e8a800ff>{4} секунды<>."
$i18n.ru.bloodsong.option = $bsTemplateRu -f $bsMin, $bsMax, $bsCd, $bsMeleeAmp, $bsDur, $bsRangedAmp
$i18n.ru.radiant_bloodsong.option = $bsTemplateRu -f $rbsMin, $rbsMax, $rbsCd, $rbsMeleeAmp, $rbsDur, $rbsRangedAmp
...
$bbRu = "$lethRu`n`nСаботаж: При участии в убийстве вражеского чемпиона даёт <#92dc7bff>Саботаж<> на <#e8a800ff>{3} секунд<>, усиливая вашу следующую базовую атаку по башне, чтобы нанести <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> вашей <$adIcon> <#ff9028ff>Силы Атаки<> как <#e3ab9dff>дополнительный чистый урон<>."
...
$spfRu = "$lethRu`n`nПожиратель щитов: Попадание базовой атакой или умением по вражескому чемпиону со щитом наносит дополнительно <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> вашей <$adIcon> <#ff9028ff>Силы Атаки<> как <#ff9028ff>дополнительный физический урон<>."
...
$oppRu = "$lethRu`n`nПодготовка: После <#e8a800ff>{1} секунд<> вне боя с вражескими чемпионами вы получаете <#ffdd8eff>{2} <$armorPenIcon> летальности<>. Этот бонус сохраняется <#e8a800ff>{3} секунд<> после нанесения урона вражескому чемпиону."
...
$lockRu = "Преданность: При <#d94c49ff>падении здоровья ниже {5}%<> вы и все союзные чемпионы поблизости получаете <#cab944ff>щит<>, поглощающий <#cab944ff>{6}<> - <#cab944ff>{7}<> (в зависимости от <$levelIcon> <#d8c9b3ff>уровня<>) <#cab944ff>урона<>, на <#e8a800ff>{8} секунды<> (перезарядка {9} секунд).`n`nЛегион: Даёт <#ffdd8eff>{0} <$armorIcon> брони<>, <#88ccffff>{1} <$mrIcon> сопротивления магии<> и <#60e84dff>{2} <$hpRegenIcon> восстановления здоровья<> всем союзникам в пределах <#ff86c2ff>{4} <$rangeIcon> дальности<>. Миньоны получают <#e8a800ff>{3}%<> от этого значения."
...
$i18n.ko.stormsurge.option = "폭풍 약탈자: <#e8a800ff>${surgeWin}초<> 내로 적 챔피언에게 <$hpIcon> <#60e84dff>최대 체력의 ${surgePct}%<>에 해당하는 피해를 입히면 질풍을 적용합니다. 대상별 재사용 대기시간은 <#e8a800ff>${surgeCd}초<>입니다.`n`n질풍: <#e8a800ff>${surgeDelay}초<> 후 번개로 대상을 강타하여 <#a974ffff>${surgeFlat}<> + <$apIcon> <#a974ffff>주문력<>의 <#a974ffff>${surgeApPct}%<>만큼 <#a974ffff>마법 피해<>를 입힙니다."
$i18n.ko.radiant_stormsurge.option = "폭풍 약탈자: <#e8a800ff>${rsurgeWin}초<> 내로 적 챔피언에게 <$hpIcon> <#60e84dff>최대 체력의 ${rsurgePct}%<>에 해당하는 피해를 입히면 질풍을 적용합니다. 대상별 재사용 대기시간은 <#e8a800ff>${rsurgeCd}초<>입니다.`n`n질풍: <#e8a800ff>${rsurgeDelay}초<> 후 번개로 대상을 강타하여 <#a974ffff>${rsurgeFlat}<> + <$apIcon> <#a974ffff>주문력<>의 <#a974ffff>${rsurgeApPct}%<>만큼 <#a974ffff>마법 피해<>를 입힙니다."
$i18n.ko.fated_ashes.option = "발화: 스킬이 적에게 적중하면 대상을 불태워 <#e8a800ff>${ashDur}초<> 동안 <#a974ffff>${ashFlat}의 마법 피해<>를 입힙니다. 이 효과는 미니언과 몬스터에게 <#e8a800ff>${ashPct}%<>로 적용됩니다."
$i18n.ko.hextech_alternator.option = "자극: 적 챔피언에게 피해를 입히면 <#a974ffff>${altFlat}의 추가 마법 피해<>를 입힙니다. 재사용 대기시간은 <#e8a800ff>${altCd}초<>입니다."
$i18n.ko.protectors_vow.option = "경외: <#60e84dff>${pvFlat}<> + <$armorIcon> <#ffdd8eff>방어력<>의 <#ffdd8eff>${pvArmorPct}%<>만큼 <$hpIcon> <#60e84dff>최대 체력<>을 얻습니다."
...
$i18n.ko.shadowflame.option = "잿불꽃: <#d94c49ff>최대 체력이 ${sfThreshold}% 아래인<> 적에게 입히는 <#a974ffff>마법 피해<>와 <#e3ab9dff>고정 피해<>가 <#e8a800ff>20% 증가<>합니다."
$i18n.ko.radiant_shadowflame.option = "잿불꽃: <#d94c49ff>최대 체력이 ${rsfThreshold}% 아래인<> 적에게 입히는 <#a974ffff>마법 피해<>와 <#e3ab9dff>고정 피해<>가 <#e8a800ff>20% 증가<>합니다."
$i18n.ko.yun_tal_wildarrows.option = "치명적인 연습: 기본 공격 시 <$critIcon> <#d45656ff>치명타 확률<>이 영구적으로 <#d45656ff>${ytCrit}%<> 증가하며, 최대 <$critIcon> <#d45656ff>${ytMaxCrit}%<>까지 증가합니다.`n`n연타: <#e8a800ff>${ytCd}초<>마다 다음 기본 공격이 <#e8a800ff>${ytDur}초<> 동안 <$asIcon> <#ceff99ff>공격 속도<>를 <#ceff99ff>${ytFlurryAS}%<> 증가시킵니다."
$i18n.ko.radiant_yun_tal_wildarrows.option = "치명적인 연습: 기본 공격 시 <$critIcon> <#d45656ff>치명타 확률<>이 영구적으로 <#d45656ff>${rytCrit}%<> 증가하며, 최대 <$critIcon> <#d45656ff>${rytMaxCrit}%<>까지 증가합니다.`n`n연타: <#e8a800ff>${rytCd}초<>마다 다음 기본 공격이 <#e8a800ff>${rytDur}초<> 동안 <$asIcon> <#ceff99ff>공격 속도<>를 <#ceff99ff>${rytFlurryAS}%<> 증가시킵니다."
$i18n.ko.phage.option = "분노: 적 챔피언에게 기본 공격을 가하면 <#e8a800ff>${phageDur}초<> 동안 <$speedIcon> <#ffffffff>이동 속도<>가 <#ffffffff>${phageMs}%<> 증가합니다."
...
$i18n.ko.rylais_crystal_scepter.option = "서리: 적에게 스킬을 적중시키면 <#e8a800ff>${rcsDur}초<> 동안 <#d94c49ff>${rcsSlow}% 둔화<>시킵니다."
$i18n.ko.radiant_rylais_crystal_scepter.option = "서리: 적에게 스킬을 적중시키면 <#e8a800ff>${rrcsDur}초<> 동안 <#d94c49ff>${rrcsSlow}% 둔화<>시킵니다."
...
$i18n.ko.blade_of_the_ruined_king.option = "안개의 검: 기본 공격 시 대상 <#d94c49ff>현재 체력의 (<$meleeIcon> ${borkMeleePct}% / <$rangedIcon> ${borkRangedPct}%)<>만큼 <#ff9028ff>추가 물리 피해<>를 입힙니다. 미니언과 몬스터에게는 최대 <#ff9028ff>${borkCap}의 물리 피해<>를 입힙니다."
$i18n.ko.radiant_blade_of_the_ruined_king.option = "안개의 검: 기본 공격 시 대상 <#d94c49ff>현재 체력의 (<$meleeIcon> ${rborkMeleePct}% / <$rangedIcon> ${rborkRangedPct}%)<>만큼 <#ff9028ff>추가 물리 피해<>를 입힙니다. 미니언과 몬스터에게는 최대 <#ff9028ff>${rborkCap}의 물리 피해<>를 입힙니다."
...
$i18n.ko.deaths_dance.option = "고통 무시: 받는 피해의 <#e8a800ff>${ddDelay}%<>를 지속적으로 <#e3ab9dff>고정 피해<>로 받습니다. 초당 피해량은 <$hpIcon> <#60e84dff>최대 체력<>의 <#60e84dff>${ddBurnCap}%<>를 넘지 않습니다.`n`n저항: 적 챔피언 처치에 관여하면 남아 있는 지연 피해를 제거하고 <$hpRegenIcon> <#60e84dff>${ddFlatHeal}<> + <#60e84dff>잃은 체력의 ${ddHeal}%<>만큼 체력을 회복합니다."
$i18n.ko.radiant_deaths_dance.option = "고통 무시: 받는 피해의 <#e8a800ff>${rddDelay}%<>를 지속적으로 <#e3ab9dff>고정 피해<>로 받습니다. 초당 피해량은 <$hpIcon> <#60e84dff>최대 체력<>의 <#60e84dff>${rddBurnCap}%<>를 넘지 않습니다.`n`n저항: 적 챔피언 처치에 관여하면 남아 있는 지연 피해를 제거하고 <$hpRegenIcon> <#60e84dff>${rddFlatHeal}<> + <#60e84dff>잃은 체력의 ${rddHeal}%<>만큼 체력을 회복합니다."
...
$i18n.ko.stormrazor.option = "충전: 이동하거나 기본 공격을 하면 최대 <#e8a800ff>${srStacks}<>까지 <#92dc7bff>충전<> 중첩을 얻습니다.`n`n벼락: 완전히 <#92dc7bff>충전<>되면 다음 기본 공격이 <#a974ffff>${srDmg}의 추가 마법 피해<>를 입히고, <#e8a800ff>${srDur}초<> 동안 <$speedIcon> <#ffffffff>이동 속도<>가 <#ffffffff>${srMs}%<> 증가합니다."
$i18n.ko.radiant_stormrazor.option = "충전: 이동하거나 기본 공격을 하면 최대 <#e8a800ff>${rsrStacks}<>까지 <#92dc7bff>충전<> 중첩을 얻습니다.`n`n벼락: 완전히 <#92dc7bff>충전<>되면 다음 기본 공격이 <#a974ffff>${rsrDmg}의 추가 마법 피해<>를 입히고, <#e8a800ff>${rsrDur}초<> 동안 <$speedIcon> <#ffffffff>이동 속도<>가 <#ffffffff>${rsrMs}%<> 증가합니다."
...
$eclKo = "늘 떠오르는 달: 적 챔피언에게 기본 공격이나 스킬을 적중시키면 <#e8a800ff>{0}초<> 동안 대상을 표식 상태로 만듭니다. 시전당 최대 1회 적용됩니다. 표식이 있는 챔피언을 적중시키면 표식을 소모하여 대상 <#d94c49ff>최대 체력의 (<$meleeIcon> {1}% / <$rangedIcon> {6}%)<>만큼 <#ff9028ff>추가 물리 피해<>를 입히고, <#e8a800ff>{4}초<> 동안 <#cab944ff>{2}<> + <$adIcon> <#ff9028ff>공격력의 {3}%<>를 흡수하는 <#cab944ff>보호막<>을 얻습니다. (대상별 재사용 대기시간 <#e8a800ff>{5}초<>)"
$i18n.ko.eclipse.option = $eclKo -f $eclDur, $eclMeleePct, $eclShield, $eclAdPct, $eclShDur, $eclCd, $eclRangedPct
$i18n.ko.radiant_eclipse.option = $eclKo -f $reclDur, $reclMeleePct, $reclShield, $reclAdPct, $reclShDur, $reclCd, $reclRangedPct
$i18n.ko.echoes_of_helia.option = "영혼 착취: 주고받은 피해의 <#e8a800ff>${eohConversion}%<>를 <#92dc7bff>영혼 충전량<>으로 저장하며, <$levelIcon> <#d8c9b3ff>레벨<>에 따라 최대 <#d8c9b3ff>${eohMinCap}<> - <#d8c9b3ff>${eohMaxCap}<>까지 저장합니다. 아군 챔피언(자신 제외)을 회복시키거나 보호막 또는 버프를 부여하면 모든 <#92dc7bff>영혼 충전량<>을 소모해 해당 아군의 체력을 소모량만큼 <$hpRegenIcon> <#60e84dff>회복<>시킵니다."
$i18n.ko.radiant_echoes_of_helia.option = "영혼 착취: 주고받은 피해의 <#e8a800ff>${reohConversion}%<>를 <#92dc7bff>영혼 충전량<>으로 저장하며, <$levelIcon> <#d8c9b3ff>레벨<>에 따라 최대 <#d8c9b3ff>${reohMinCap}<> - <#d8c9b3ff>${reohMaxCap}<>까지 저장합니다. 아군 챔피언(자신 제외)을 회복시키거나 보호막 또는 버프를 부여하면 모든 <#92dc7bff>영혼 충전량<>을 소모해 해당 아군의 체력을 소모량만큼 <$hpRegenIcon> <#60e84dff>회복<>시킵니다."
...
$rhTemplateKo = "쪼개기: 기본 공격 시 대상 주변 <$rangeIcon> <#ff86c2ff>사거리 {2}<> 안의 적들에게 <$adIcon> <#ff9028ff>공격력의<> <#ff9028ff>(<$meleeIcon> {0}% / <$rangedIcon> {1}%)<>만큼 <#ff9028ff>물리 피해<>를 입힙니다."
$i18n.ko.ravenous_hydra.option = $rhTemplateKo -f $rhMeleePct, $rhRangedPct, $rhRange
$i18n.ko.radiant_ravenous_hydra.option = $rhTemplateKo -f $rrhMeleePct, $rrhRangedPct, $rrhRange
...
$bsTemplateKo = "주문 검: 적 챔피언에게 스킬을 적중시키면 다음 기본 공격이 <$levelIcon> <#d8c9b3ff>레벨<>에 따라 <#a974ffff>{0}<>~<#a974ffff>{1}<>의 <#a974ffff>추가 마법 피해<>를 입힙니다. (재사용 대기시간 <#e8a800ff>{2}초<>) 대상이 챔피언이면 <#e8a800ff>{4}초<> 동안 대상이 <#d94c49ff>받는 피해<>가 <#d94c49ff>(<$meleeIcon> {3}% / <$rangedIcon> {5}%)<> 증가합니다."
$i18n.ko.bloodsong.option = $bsTemplateKo -f $bsMin, $bsMax, $bsCd, $bsMeleeAmp, $bsDur, $bsRangedAmp
$i18n.ko.radiant_bloodsong.option = $bsTemplateKo -f $rbsMin, $rbsMax, $rbsCd, $rbsMeleeAmp, $rbsDur, $rbsRangedAmp
...
$bbKo = "$lethKo`n`n파괴 공작: 적 챔피언 처치에 관여하면 <#e8a800ff>{3}초<> 동안 <#92dc7bff>파괴 공작<>을 얻습니다. 이 효과를 보유한 동안 포탑을 대상으로 하는 다음 기본 공격이 <#ff9028ff>{1}<> + <$adIcon> <#ff9028ff>공격력의 {2}%<>만큼 <#e3ab9dff>추가 고정 피해<>를 입힙니다."
...
$spfKo = "$lethKo`n`n보호막 파괴자: 보호막이 있는 적 챔피언에게 기본 공격이나 스킬을 적중시키면 <#ff9028ff>{1}<> + <$adIcon> <#ff9028ff>공격력의 {2}%<>만큼 <#ff9028ff>추가 물리 피해<>를 입힙니다."
...
$oppKo = "$lethKo`n`n준비: 적 챔피언과 <#e8a800ff>{1}초<> 동안 비전투 상태를 유지하면 <#ffdd8eff>{2}의 <$armorPenIcon> 물리 관통력<>을 획득합니다. 이 효과는 적 챔피언에게 피해를 입힌 후 <#e8a800ff>{3}초<> 동안 유지됩니다."
...
$lockKo = "헌신: <#d94c49ff>체력이 {5}% 아래로 내려가면<> 자신과 주변의 모든 아군 챔피언이 <#e8a800ff>{8}초<> 동안 <$levelIcon> <#d8c9b3ff>레벨<>에 따라 <#cab944ff>{6}<>~<#cab944ff>{7}<>의 <#cab944ff>피해를 흡수<>하는 <#cab944ff>보호막<>을 얻습니다. 재사용 대기시간은 <#e8a800ff>{9}초<>입니다.`n`n군단: <$rangeIcon> <#ff86c2ff>사거리 {4}<> 안의 모든 아군에게 <$armorIcon> <#ffdd8eff>방어력 {0}<>, <$mrIcon> <#88ccffff>마법 저항력 {1}<>, <$hpRegenIcon> <#60e84dff>체력 재생 {2}<>를 부여합니다. 미니언은 이 수치의 <#e8a800ff>{3}%<>를 얻습니다."
...
$dmpKo = "난파선: 이동하는 동안 매초 <#92dc7bff>여세<>를 <#e8a800ff>{0}중첩<> 얻으며, 최대 <#e8a800ff>{1}중첩<>까지 쌓입니다. 중첩당 <$speedIcon> <#ffffffff>이동 속도 {2}%<>를 얻습니다. 기본 공격 시 남은 <#92dc7bff>여세<>를 모두 소모하여 소모한 중첩에 비례해 <#ff9028ff>{3} - {4}의 추가 물리 피해<>를 입힙니다."
...
Write-Host "  Blade of the Ruined King:         ${borkMeleePct}%/${borkRangedPct}% HP / ${borkCap} cap"
Write-Host "  Radiant Blade of the Ruined King: ${rborkMeleePct}%/${rborkRangedPct}% HP / ${rborkCap} cap"
...
Write-Host "  Eclipse:                     ${eclMeleePct}%/${eclRangedPct}% max HP dmg / ${eclShield} + ${eclAdPct}% AD shield ${eclShDur}s / ${eclDur}s mark / ${eclCd}s CD"
Write-Host "  Radiant Eclipse:             ${reclMeleePct}%/${reclRangedPct}% max HP dmg / ${reclShield} + ${reclAdPct}% AD shield ${reclShDur}s / ${reclDur}s mark / ${reclCd}s CD"
Write-Host "  Bloodsong:                   ${bsMin} - ${bsMax} magic (by level) / ${bsCd}s CD / +${bsMeleeAmp}% melee / +${bsRangedAmp}% ranged damage taken ${bsDur}s"
Write-Host "  Radiant Bloodsong:           ${rbsMin} - ${rbsMax} magic (by level) / ${rbsCd}s CD / +${rbsMeleeAmp}% melee / +${rbsRangedAmp}% ranged damage taken ${rbsDur}s"
