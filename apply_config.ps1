...
$hmacDmg = [int]$config.hunters_machete.effect_bonus_magic_damage
$hmacHeal = [int]$config.hunters_machete.effect_bonus_flat_heal
$mrazDmg = [int]$config.madreds_razors.effect_bonus_magic_damage
$mrazHeal = [int]$config.madreds_razors.effect_bonus_flat_heal
$wrigDmg = [int]$config.wriggles_lantern.effect_bonus_magic_damage
$wrigHeal = [int]$config.wriggles_lantern.effect_bonus_flat_heal
...
$htalBonus = [int]$config.hunters_talisman.effect_percent_bonus_damage
$htalHeal = [int]$config.hunters_talisman.effect_bonus_hp_percent_of_damage
$sstoneBonus = [int]$config.spirit_stone.effect_percent_bonus_damage
$sstoneHeal = [int]$config.spirit_stone.effect_bonus_hp_percent_of_damage
$gslBonus = [int]$config.grezs_spectral_lantern.effect_percent_bonus_damage
$gslHeal = [int]$config.grezs_spectral_lantern.effect_bonus_hp_percent_of_damage
$sswAp = [int]$config.spirit_of_the_spectral_wraith.effect_stack_magic_power
$sswStacks = [int]$config.spirit_of_the_spectral_wraith.effect_max_stacks
$sswMaxAp = $sswAp * $sswStacks
$sswBonus = [int]$config.spirit_of_the_spectral_wraith.effect_percent_bonus_damage
$sswHeal = [int]$config.spirit_of_the_spectral_wraith.effect_bonus_hp_percent_of_damage
$rsswAp = [int]$config.radiant_spirit_of_the_spectral_wraith.effect_stack_magic_power
$rsswStacks = [int]$config.radiant_spirit_of_the_spectral_wraith.effect_max_stacks
$rsswMaxAp = $rsswAp * $rsswStacks
$rsswBonus = [int]$config.radiant_spirit_of_the_spectral_wraith.effect_percent_bonus_damage
$rsswHeal = [int]$config.radiant_spirit_of_the_spectral_wraith.effect_bonus_hp_percent_of_damage
...
$maimEn = "Maim: Basic attacks against monsters deal <#a974ffff>{0} bonus magic damage<> and restore <#60e84dff>{1} <$hpIcon> health<>."
$i18n.en.hunters_machete.option = $maimEn -f $hmacDmg, $hmacHeal
$i18n.en.madreds_razors.option = $maimEn -f $mrazDmg, $mrazHeal
$i18n.en.wriggles_lantern.option = $maimEn -f $wrigDmg, $wrigHeal
...
$butcherEn = "Butcher: Deal <#d94c49ff>{0}% bonus damage<> against monsters and <$hpRegenIcon> <#60e84dff>heal<> for <#d94c49ff>{1}% of the damage dealt<>."
$i18n.en.hunters_talisman.option = $butcherEn -f $htalBonus, $htalHeal
$i18n.en.spirit_stone.option = $butcherEn -f $sstoneBonus, $sstoneHeal
$i18n.en.grezs_spectral_lantern.option = $butcherEn -f $gslBonus, $gslHeal
$sswEn = "Spirit Drain: Grants <#a974ffff>{0} <$apIcon> Ability Power<> for each champion takedown, up to <#a974ffff>{1}<> <$apIcon>.`n`n" + $butcherEn.Replace("{0}", "{2}").Replace("{1}", "{3}")
$i18n.en.spirit_of_the_spectral_wraith.option = $sswEn -f $sswAp, $sswMaxAp, $sswBonus, $sswHeal
$i18n.en.radiant_spirit_of_the_spectral_wraith.option = $sswEn -f $rsswAp, $rsswMaxAp, $rsswBonus, $rsswHeal
...
$eclEn = "Ever Rising Moon: Landing a basic attack or an Ability on an enemy champion <#92dc7bff>marks<> them for <#e8a800ff>{0} seconds<>, up to once per cast. Hitting a champion that is already <#92dc7bff>marked<> consumes the mark to deal <#ff9028ff>bonus physical damage<> equal to <#d94c49ff>{1}% of their maximum health<> and grant you a <#cab944ff>shield<> of <#d8c9b3ff>{2}<> + <#d8c9b3ff>{3}%<> of your <$adIcon> <#ff9028ff>Attack Damage<> for <#e8a800ff>{4} seconds<> (<#e8a800ff>{5} second<> cooldown)."
...
$bbEn = "$lethEn`n`nSabotage: Scoring a <#e8a800ff>takedown<> on an enemy champion grants <#92dc7bff>Sabotage<> for <#e8a800ff>{3} seconds<>, empowering your next basic attack against a turret to deal <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> of your <$adIcon> <#ff9028ff>Attack Damage<> as <#ff9028ff>bonus physical damage<>."
...
$maimVi = "Tàn Phế: Đòn đánh thường lên quái gây <#a974ffff>{0} sát thương phép cộng thêm<> và hồi <#60e84dff>{1} <$hpIcon> máu<>."
$i18n.vi.hunters_machete.option = $maimVi -f $hmacDmg, $hmacHeal
$i18n.vi.madreds_razors.option = $maimVi -f $mrazDmg, $mrazHeal
$i18n.vi.wriggles_lantern.option = $maimVi -f $wrigDmg, $wrigHeal
...
$butcherVi = "Đồ Tể: Gây <#d94c49ff>{0}% sát thương cộng thêm<> lên quái và <$hpRegenIcon> <#60e84dff>hồi máu<> bằng <#d94c49ff>{1}% sát thương đã gây<>."
$i18n.vi.hunters_talisman.option = $butcherVi -f $htalBonus, $htalHeal
$i18n.vi.spirit_stone.option = $butcherVi -f $sstoneBonus, $sstoneHeal
$i18n.vi.grezs_spectral_lantern.option = $butcherVi -f $gslBonus, $gslHeal
$sswVi = "Hút Linh Hồn: Nhận <#a974ffff>{0} <$apIcon> SMPT<> mỗi khi tham gia hạ gục một tướng địch, tối đa <#a974ffff>{1}<> <$apIcon>.`n`n" + $butcherVi.Replace("{0}", "{2}").Replace("{1}", "{3}")
$i18n.vi.spirit_of_the_spectral_wraith.option = $sswVi -f $sswAp, $sswMaxAp, $sswBonus, $sswHeal
$i18n.vi.radiant_spirit_of_the_spectral_wraith.option = $sswVi -f $rsswAp, $rsswMaxAp, $rsswBonus, $rsswHeal
...
$eclVi = "Ánh Trăng Công Kích: Đòn đánh thường hoặc kĩ năng trúng tướng địch sẽ <#92dc7bff>đánh dấu<> chúng trong <#e8a800ff>{0} giây<>, tối đa một lần mỗi lần thi triển. Đánh trúng tướng đã bị <#92dc7bff>đánh dấu<> sẽ tiêu thụ dấu, gây <#ff9028ff>sát thương vật lí cộng thêm<> bằng <#d94c49ff>{1}% máu tối đa của chúng<> và tạo cho bạn một <#cab944ff>khiên<> bằng <#d8c9b3ff>{2}<> + <#d8c9b3ff>{3}%<> <$adIcon> <#ff9028ff>SMCK<> của bạn trong <#e8a800ff>{4} giây<> (hồi chiêu <#e8a800ff>{5} giây<>)."
...
$bbVi = "$lethVi`n`nPhá Hoại: Tham gia hạ gục một tướng địch sẽ trao <#92dc7bff>Phá Hoại<> trong <#e8a800ff>{3} giây<>, tăng cường đòn đánh thường tiếp theo của bạn lên trụ để gây <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> <$adIcon> <#ff9028ff>SMCK<> dưới dạng <#ff9028ff>sát thương vật lí cộng thêm<>."
...
$maimZh = "致残：普通攻击野怪时额外造成 <#a974ffff>{0} 点魔法伤害<>并回复 <#60e84dff>{1} 点<$hpIcon> 生命值<>。"
$i18n.'zh-hans'.hunters_machete.option = $maimZh -f $hmacDmg, $hmacHeal
$i18n.'zh-hans'.madreds_razors.option = $maimZh -f $mrazDmg, $mrazHeal
$i18n.'zh-hans'.wriggles_lantern.option = $maimZh -f $wrigDmg, $wrigHeal
...
$butcherZh = "屠戮：对野怪造成 <#d94c49ff>{0}% 额外伤害<>，并<$hpRegenIcon> <#60e84dff>回复<>相当于<#d94c49ff>已造成伤害 {1}%<>的生命值。"
$i18n.'zh-hans'.hunters_talisman.option = $butcherZh -f $htalBonus, $htalHeal
$i18n.'zh-hans'.spirit_stone.option = $butcherZh -f $sstoneBonus, $sstoneHeal
$i18n.'zh-hans'.grezs_spectral_lantern.option = $butcherZh -f $gslBonus, $gslHeal
$sswZh = "灵魂汲取：每次参与击杀敌方英雄获得 <#a974ffff>{0} 点<$apIcon> 法术强度<>，最多 <#a974ffff>{1}<> 点<$apIcon>。`n`n" + $butcherZh.Replace("{0}", "{2}").Replace("{1}", "{3}")
$i18n.'zh-hans'.spirit_of_the_spectral_wraith.option = $sswZh -f $sswAp, $sswMaxAp, $sswBonus, $sswHeal
$i18n.'zh-hans'.radiant_spirit_of_the_spectral_wraith.option = $sswZh -f $rsswAp, $rsswMaxAp, $rsswBonus, $rsswHeal
...
$eclZh = "永升之月：普通攻击或技能命中敌方英雄时会<#92dc7bff>标记<>其 <#e8a800ff>{0}秒<>，每次施放最多触发一次。命中已被<#92dc7bff>标记<>的英雄会消耗该标记，造成相当于<#d94c49ff>其{1}%最大生命值<>的<#ff9028ff>额外物理伤害<>，并为你获得相当于 <#d8c9b3ff>{2}<> + <$adIcon> <#ff9028ff>攻击力<>的 <#d8c9b3ff>{3}%<> 的<#cab944ff>护盾<>，持续 <#e8a800ff>{4}秒<>（冷却 <#e8a800ff>{5}秒<>）。"
...
$bbZh = "$lethZh`n`n破坏：参与击杀敌方英雄时，获得 <#92dc7bff>破坏<> 效果，持续 <#e8a800ff>{3}秒<>，强化你对防御塔的下一次普通攻击，造成 <#ff9028ff>{1}<> + <$adIcon> <#ff9028ff>攻击力<>的 <#ff9028ff>{2}%<> 作为<#ff9028ff>额外物理伤害<>。"
...
$maimPt = "Mutilar: Ataques básicos contra monstros causam <#a974ffff>{0} de dano mágico bônus<> e restauram <#60e84dff>{1} de <$hpIcon> vida<>."
$i18n.'pt-BR'.hunters_machete.option = $maimPt -f $hmacDmg, $hmacHeal
$i18n.'pt-BR'.madreds_razors.option = $maimPt -f $mrazDmg, $mrazHeal
$i18n.'pt-BR'.wriggles_lantern.option = $maimPt -f $wrigDmg, $wrigHeal
...
$butcherPt = "Açougueiro: Causa <#d94c49ff>{0}% de dano bônus<> contra monstros e <$hpRegenIcon> <#60e84dff>cura<> <#d94c49ff>{1}% do dano causado<>."
$i18n.'pt-BR'.hunters_talisman.option = $butcherPt -f $htalBonus, $htalHeal
$i18n.'pt-BR'.spirit_stone.option = $butcherPt -f $sstoneBonus, $sstoneHeal
$i18n.'pt-BR'.grezs_spectral_lantern.option = $butcherPt -f $gslBonus, $gslHeal
$sswPt = "Dreno Espiritual: Concede <#a974ffff>{0} de <$apIcon> Poder de Habilidade<> ao participar do abate de um campeão inimigo, até <#a974ffff>{1}<> <$apIcon>.`n`n" + $butcherPt.Replace("{0}", "{2}").Replace("{1}", "{3}")
$i18n.'pt-BR'.spirit_of_the_spectral_wraith.option = $sswPt -f $sswAp, $sswMaxAp, $sswBonus, $sswHeal
$i18n.'pt-BR'.radiant_spirit_of_the_spectral_wraith.option = $sswPt -f $rsswAp, $rsswMaxAp, $rsswBonus, $rsswHeal
...
$eclPt = "Lua Sempre Crescente: Acertar um ataque básico ou uma Habilidade em um campeão inimigo o <#92dc7bff>marca<> por <#e8a800ff>{0} segundos<>, no máximo uma vez por conjuração. Atingir um campeão que já esteja <#92dc7bff>marcado<> consome a marca para causar <#ff9028ff>dano físico bônus<> equivalente a <#d94c49ff>{1}% da Vida Máxima dele<> e conceder a você um <#cab944ff>escudo<> de <#d8c9b3ff>{2}<> + <#d8c9b3ff>{3}%<> do seu <$adIcon> <#ff9028ff>Dano de Ataque<> por <#e8a800ff>{4} segundos<> (recarga de <#e8a800ff>{5} segundos<>)."
...
$bbPt = "$lethPt`n`nSabotagem: Ao participar do abate de um campeão inimigo, concede <#92dc7bff>Sabotagem<> por <#e8a800ff>{3} segundos<>, potencializando seu próximo ataque básico contra uma torre para causar <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> do seu <$adIcon> <#ff9028ff>Dano de Ataque<> como <#ff9028ff>dano físico bônus<>."
...
$maimRu = "Увечье: Автоатаки по монстрам наносят <#a974ffff>{0} дополнительного магического урона<> и восстанавливают <#60e84dff>{1} <$hpIcon> здоровья<>."
$i18n.ru.hunters_machete.option = $maimRu -f $hmacDmg, $hmacHeal
$i18n.ru.madreds_razors.option = $maimRu -f $mrazDmg, $mrazHeal
$i18n.ru.wriggles_lantern.option = $maimRu -f $wrigDmg, $wrigHeal
...
$butcherRu = "Мясник: Наносит <#d94c49ff>{0}% дополнительного урона<> монстрам и <$hpRegenIcon> <#60e84dff>восстанавливает<> <#d94c49ff>{1}% нанесённого урона<>."
$i18n.ru.hunters_talisman.option = $butcherRu -f $htalBonus, $htalHeal
$i18n.ru.spirit_stone.option = $butcherRu -f $sstoneBonus, $sstoneHeal
$i18n.ru.grezs_spectral_lantern.option = $butcherRu -f $gslBonus, $gslHeal
$sswRu = "Похищение духа: При участии в убийстве вражеского чемпиона даёт <#a974ffff>{0} <$apIcon> Силы умений<>, до <#a974ffff>{1}<> <$apIcon>.`n`n" + $butcherRu.Replace("{0}", "{2}").Replace("{1}", "{3}")
$i18n.ru.spirit_of_the_spectral_wraith.option = $sswRu -f $sswAp, $sswMaxAp, $sswBonus, $sswHeal
$i18n.ru.radiant_spirit_of_the_spectral_wraith.option = $sswRu -f $rsswAp, $rsswMaxAp, $rsswBonus, $rsswHeal
...
$eclRu = "Восходящая луна: Попадание базовой атакой или умением по вражескому чемпиону <#92dc7bff>отмечает<> его на <#e8a800ff>{0} секунды<>, не более одного раза за применение. Попадание по чемпиону, который уже <#92dc7bff>отмечен<>, поглощает метку, нанося <#ff9028ff>дополнительный физический урон<> в размере <#d94c49ff>{1}% его максимального здоровья<> и давая вам <#cab944ff>щит<> в размере <#d8c9b3ff>{2}<> + <#d8c9b3ff>{3}%<> вашей <$adIcon> <#ff9028ff>силы атаки<> на <#e8a800ff>{4} секунды<> (перезарядка <#e8a800ff>{5} секунд<>)."
...
$bbRu = "$lethRu`n`nСаботаж: При участии в убийстве вражеского чемпиона даёт <#92dc7bff>Саботаж<> на <#e8a800ff>{3} секунд<>, усиливая вашу следующую базовую атаку по башне, чтобы нанести <#ff9028ff>{1}<> + <#ff9028ff>{2}%<> вашей <$adIcon> <#ff9028ff>Силы Атаки<> как <#ff9028ff>дополнительный физический урон<>."
...
$maimKo = "불구: 몬스터를 기본 공격하면 <#a974ffff>{0}의 추가 마법 피해<>를 입히고 <#60e84dff>{1}<$hpIcon> 체력<>을 회복합니다."
$i18n.ko.hunters_machete.option = $maimKo -f $hmacDmg, $hmacHeal
$i18n.ko.madreds_razors.option = $maimKo -f $mrazDmg, $mrazHeal
$i18n.ko.wriggles_lantern.option = $maimKo -f $wrigDmg, $wrigHeal
...
$butcherKo = "도살자: 몬스터에게 <#d94c49ff>{0}%의 추가 피해<>를 입히고 <#d94c49ff>입힌 피해의 {1}%<>만큼 <$hpRegenIcon> <#60e84dff>체력을 회복<>합니다."
$i18n.ko.hunters_talisman.option = $butcherKo -f $htalBonus, $htalHeal
$i18n.ko.spirit_stone.option = $butcherKo -f $sstoneBonus, $sstoneHeal
$i18n.ko.grezs_spectral_lantern.option = $butcherKo -f $gslBonus, $gslHeal
$sswKo = "영혼 흡수: 적 챔피언 처치에 관여할 때마다 <#a974ffff>{0}의 <$apIcon> 주문력<>을 얻으며, 최대 <#a974ffff>{1}<> <$apIcon>까지 쌓입니다.`n`n" + $butcherKo.Replace("{0}", "{2}").Replace("{1}", "{3}")
$i18n.ko.spirit_of_the_spectral_wraith.option = $sswKo -f $sswAp, $sswMaxAp, $sswBonus, $sswHeal
$i18n.ko.radiant_spirit_of_the_spectral_wraith.option = $sswKo -f $rsswAp, $rsswMaxAp, $rsswBonus, $rsswHeal
...
$eclKo = "늘 떠오르는 달: 적 챔피언에게 기본 공격이나 스킬을 적중시키면 <#e8a800ff>{0}초<> 동안 대상을 <#92dc7bff>표식<> 상태로 만듭니다. 시전당 최대 1회 적용됩니다. 이미 <#92dc7bff>표식<>이 있는 챔피언을 적중시키면 표식을 소모하여 대상 <#d94c49ff>최대 체력의 {1}%<>만큼 <#ff9028ff>추가 물리 피해<>를 입히고, <#e8a800ff>{4}초<> 동안 <#d8c9b3ff>{2}<> + <$adIcon> <#ff9028ff>공격력의 {3}%<>만큼 <#cab944ff>피해를 흡수<>하는 <#cab944ff>보호막<>을 얻습니다. (재사용 대기시간 <#e8a800ff>{5}초<>)"
