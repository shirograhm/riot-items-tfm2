...
$i18n.vi.stormsurge.option = "Đột Kích Bão Tố: Gây sát thương bằng <#60e84dff>${surgePct}%<> <$hpIcon> <#60e84dff>Máu tối đa<> của tướng địch trong vòng <#e8a800ff>${surgeWin} giây<> sẽ gây hiệu ứng Gió Giật lên chúng (hồi chiêu <#e8a800ff>${surgeCd} giây<> mỗi mục tiêu).`n`nGió Giật: Sau <#e8a800ff>${surgeDelay} giây<>, đánh vào mục tiêu, gây <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> <$apIcon> <#a974ffff>SMPT<> dưới dạng <#a974ffff>sát thương phép<>."
$i18n.vi.radiant_stormsurge.option = "Đột Kích Bão Tố: Gây sát thương bằng <#60e84dff>${rsurgePct}%<> <$hpIcon> <#60e84dff>Máu tối đa<> của tướng địch trong vòng <#e8a800ff>${rsurgeWin} giây<> sẽ gây hiệu ứng Gió Giật lên chúng (hồi chiêu <#e8a800ff>${rsurgeCd} giây<> mỗi mục tiêu).`n`nGió Giật: Sau <#e8a800ff>${rsurgeDelay} giây<>, đánh vào mục tiêu, gây <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> <$apIcon> <#a974ffff>SMPT<> dưới dạng <#a974ffff>sát thương phép<>."
...
$i18n.'zh-hans'.stormsurge.option = "风暴掠袭：在 <#e8a800ff>${surgeWin}秒<> 内对敌方英雄造成相当于其 <#60e84dff>${surgePct}%<> <$hpIcon> <#60e84dff>最大生命值<> 的伤害时，对其施加风啸（每个目标冷却 <#e8a800ff>${surgeCd}秒<>）。`n`n风啸：<#e8a800ff>${surgeDelay}秒<> 后打击目标，造成 <#a974ffff>${surgeFlat}<> + <#a974ffff>${surgeApPct}%<> <$apIcon> <#a974ffff>法术强度<> 的<#a974ffff>魔法伤害<>。"
$i18n.'zh-hans'.radiant_stormsurge.option = "风暴掠袭：在 <#e8a800ff>${rsurgeWin}秒<> 内对敌方英雄造成相当于其 <#60e84dff>${rsurgePct}%<> <$hpIcon> <#60e84dff>最大生命值<> 的伤害时，对其施加风啸（每个目标冷却 <#e8a800ff>${rsurgeCd}秒<>）。`n`n风啸：<#e8a800ff>${rsurgeDelay}秒<> 后打击目标，造成 <#a974ffff>${rsurgeFlat}<> + <#a974ffff>${rsurgeApPct}%<> <$apIcon> <#a974ffff>法术强度<> 的<#a974ffff>魔法伤害<>。"
...
$i18n.ko.stormsurge.option = "폭풍 약탈자: <#e8a800ff>${surgeWin}초<> 내로 적 챔피언에게 <$hpIcon> <#60e84dff>최대 체력의 ${surgePct}%<>에 해당하는 피해를 입히면 질풍을 적용합니다. 대상별 재사용 대기시간은 <#e8a800ff>${surgeCd}초<>입니다.`n`n질풍: <#e8a800ff>${surgeDelay}초<> 후 대상을 강타하여 <#a974ffff>${surgeFlat}<> + <$apIcon> <#a974ffff>주문력<>의 <#a974ffff>${surgeApPct}%<>만큼 <#a974ffff>마법 피해<>를 입힙니다."
$i18n.ko.radiant_stormsurge.option = "폭풍 약탈자: <#e8a800ff>${rsurgeWin}초<> 내로 적 챔피언에게 <$hpIcon> <#60e84dff>최대 체력의 ${rsurgePct}%<>에 해당하는 피해를 입히면 질풍을 적용합니다. 대상별 재사용 대기시간은 <#e8a800ff>${rsurgeCd}초<>입니다.`n`n질풍: <#e8a800ff>${rsurgeDelay}초<> 후 대상을 강타하여 <#a974ffff>${rsurgeFlat}<> + <$apIcon> <#a974ffff>주문력<>의 <#a974ffff>${rsurgeApPct}%<>만큼 <#a974ffff>마법 피해<>를 입힙니다."
