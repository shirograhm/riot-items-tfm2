...
$dmpMinDmg = [int]$config.dead_mans_plate.effect_bonus_flat_damage
$dmpMaxDmg = [int][Math]::Round($dmpMinDmg * (1 + [double]$config.dead_mans_plate.effect_stack_damage_percent / 100 * $dmpMax))
...
$rdmpMinDmg = [int]$config.radiant_dead_mans_plate.effect_bonus_flat_damage
$rdmpMaxDmg = [int][Math]::Round($rdmpMinDmg * (1 + [double]$config.radiant_dead_mans_plate.effect_stack_damage_percent / 100 * $rdmpMax))
...
$oppEn = "$lethEn`n`nPreparation: After being out-of-combat with enemy champions for <#e8a800ff>{1} seconds<>, increase Lethality's ignored armor by <#ffdd8eff>{2} <$armorIcon> armor<>. This bonus remains for <#e8a800ff>{3} seconds<> after dealing damage to an enemy champion."
...
$lockEn = "Legion: Nearby allies gain <#ffdd8eff>{0} <$armorIcon> bonus armor<>, <#88ccffff>{1} <$mrIcon> bonus magic resistance<>, and <#60e84dff>{2} <$hpRegenIcon> health regeneration<>. Minions gain <#e8a800ff>{3}%<> of this value."
$i18n.en.locket_of_the_iron_solari.option = $lockEn -f $lockDef, $lockMr, $lockRegen, $lockMinion
$i18n.en.radiant_locket_of_the_iron_solari.option = $lockEn -f $rlockDef, $rlockMr, $rlockRegen, $rlockMinion
$dmpEn = "Shipwrecker: While moving, generate <#e8a800ff>{0} stacks<> of Momentum every second, up to <#e8a800ff>{1}<>. Each stack grants <#ffffffff>{2}%<> <$speedIcon> <#ffffffff>bonus movement speed<>. Dealing <#ff9028ff>physical damage<> to an enemy champion consumes all remaining stacks to deal <#ff9028ff>{3} - {4} bonus physical damage<>, scaling with Momentum consumed."
...
$oppVi = "$lethVi`n`nChuẩn Bị: Sau khi ở ngoài giao tranh với tướng địch trong <#e8a800ff>{1} giây<>, lượng giáp bị bỏ qua của Xuyên Giáp Trắng tăng thêm <#ffdd8eff>{2} <$armorIcon> giáp<>. Hiệu ứng này duy trì thêm <#e8a800ff>{3} giây<> sau khi gây sát thương lên tướng địch."
...
$lockVi = "Quân Đoàn: Đồng minh xung quanh nhận thêm <#ffdd8eff>{0} <$armorIcon> giáp<>, <#88ccffff>{1} <$mrIcon> kháng phép<> và <#60e84dff>{2} <$hpRegenIcon> hồi máu mỗi giây<>. Lính nhận <#e8a800ff>{3}%<> giá trị này."
$i18n.vi.locket_of_the_iron_solari.option = $lockVi -f $lockDef, $lockMr, $lockRegen, $lockMinion
$i18n.vi.radiant_locket_of_the_iron_solari.option = $lockVi -f $rlockDef, $rlockMr, $rlockRegen, $rlockMinion
$dmpVi = "Kẻ Đắm Tàu: Khi di chuyển, tích <#e8a800ff>{0} cộng dồn<> Đà mỗi giây, tối đa <#e8a800ff>{1}<>. Mỗi cộng dồn tăng <#ffffffff>{2}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<>. Gây <#ff9028ff>sát thương vật lí<> lên tướng địch sẽ tiêu hết số cộng dồn còn lại để gây <#ff9028ff>{3} - {4} sát thương vật lí cộng thêm<>, tăng theo lượng Đà đã tiêu."
...
$oppZh = "$lethZh`n`n伺机待发：脱离与敌方英雄的战斗达 <#e8a800ff>{1}秒<> 后，穿甲无视的护甲提高 <#ffdd8eff>{2} 点<$armorIcon> 护甲<>。该加成在对敌方英雄造成伤害后保留 <#e8a800ff>{3}秒<>。"
...
$lockZh = "军团：附近友军获得 <#ffdd8eff>{0} 点<$armorIcon> 护甲<>、<#88ccffff>{1} 点<$mrIcon> 魔法抗性<>和 <#60e84dff>{2} 点<$hpRegenIcon> 生命回复<>。小兵获得该数值的 <#e8a800ff>{3}%<>。"
$i18n.'zh-hans'.locket_of_the_iron_solari.option = $lockZh -f $lockDef, $lockMr, $lockRegen, $lockMinion
$i18n.'zh-hans'.radiant_locket_of_the_iron_solari.option = $lockZh -f $rlockDef, $rlockMr, $rlockRegen, $rlockMinion
$dmpZh = "碎船者：移动时每秒获得 <#e8a800ff>{0} 层<>动量，最多 <#e8a800ff>{1}<> 层。每层提供 <#ffffffff>{2}%<> <$speedIcon> <#ffffffff>额外移动速度<>。对敌方英雄造成<#ff9028ff>物理伤害<>时消耗所有剩余层数，造成 <#ff9028ff>{3} - {4} 点额外物理伤害<>，随消耗的动量提升。"
...
$oppPt = "$lethPt`n`nPreparação: Após ficar fora de combate com campeões inimigos por <#e8a800ff>{1} segundos<>, a Armadura ignorada pela Letalidade aumenta em <#ffdd8eff>{2} de <$armorIcon> Armadura<>. Este bônus permanece por <#e8a800ff>{3} segundos<> após causar dano a um campeão inimigo."
...
$lockPt = "Legião: Aliados próximos recebem <#ffdd8eff>{0} de <$armorIcon> Armadura<>, <#88ccffff>{1} de <$mrIcon> Resistência Mágica<> e <#60e84dff>{2} de <$hpRegenIcon> Regeneração de Vida<>. Tropas recebem <#e8a800ff>{3}%<> deste valor."
$i18n.'pt-BR'.locket_of_the_iron_solari.option = $lockPt -f $lockDef, $lockMr, $lockRegen, $lockMinion
$i18n.'pt-BR'.radiant_locket_of_the_iron_solari.option = $lockPt -f $rlockDef, $rlockMr, $rlockRegen, $rlockMinion
$dmpPt = "Naufragador: Ao se mover, gere <#e8a800ff>{0} cargas<> de Ímpeto por segundo, até <#e8a800ff>{1}<>. Cada carga concede <#ffffffff>{2}%<> de <$speedIcon> <#ffffffff>Velocidade de Movimento bônus<>. Causar <#ff9028ff>dano físico<> a um campeão inimigo consome todas as cargas restantes para causar <#ff9028ff>{3} - {4} de dano físico bônus<>, conforme o Ímpeto consumido."
...
$oppRu = "$lethRu`n`nПодготовка: После <#e8a800ff>{1} секунд<> вне боя с вражескими чемпионами Летальность игнорирует дополнительно <#ffdd8eff>{2} <$armorIcon> брони<>. Этот бонус сохраняется <#e8a800ff>{3} секунд<> после нанесения урона вражескому чемпиону."
...
$lockRu = "Легион: Союзники поблизости получают <#ffdd8eff>{0} <$armorIcon> брони<>, <#88ccffff>{1} <$mrIcon> сопротивления магии<> и <#60e84dff>{2} <$hpRegenIcon> восстановления здоровья<>. Миньоны получают <#e8a800ff>{3}%<> от этого значения."
$i18n.ru.locket_of_the_iron_solari.option = $lockRu -f $lockDef, $lockMr, $lockRegen, $lockMinion
$i18n.ru.radiant_locket_of_the_iron_solari.option = $lockRu -f $rlockDef, $rlockMr, $rlockRegen, $rlockMinion
$dmpRu = "Крушитель: При движении вы получаете <#e8a800ff>{0} зарядов<> Инерции в секунду, до <#e8a800ff>{1}<>. Каждый заряд дает <#ffffffff>{2}%<> <$speedIcon> <#ffffffff>дополнительной скорости передвижения<>. Нанося <#ff9028ff>физический урон<> вражескому чемпиону, вы тратите все накопленные заряды и наносите <#ff9028ff>{3} - {4} дополнительного физического урона<> в зависимости от потраченной Инерции."
...
$oppKo = "$lethKo`n`n준비: 적 챔피언과 <#e8a800ff>{1}초<> 동안 비전투 상태를 유지하면 물리 관통력의 <$armorIcon> 방어력 무시량이 <#ffdd8eff>{2}<> 증가합니다. 이 효과는 적 챔피언에게 피해를 입힌 후 <#e8a800ff>{3}초<> 동안 유지됩니다."
...
$lockKo = "군단: 주변 아군이 <$armorIcon> <#ffdd8eff>방어력 {0}<>, <$mrIcon> <#88ccffff>마법 저항력 {1}<>, <$hpRegenIcon> <#60e84dff>체력 재생 {2}<>를 얻습니다. 미니언은 이 수치의 <#e8a800ff>{3}%<>를 얻습니다."
$i18n.ko.locket_of_the_iron_solari.option = $lockKo -f $lockDef, $lockMr, $lockRegen, $lockMinion
$i18n.ko.radiant_locket_of_the_iron_solari.option = $lockKo -f $rlockDef, $rlockMr, $rlockRegen, $rlockMinion
$dmpKo = "난파선: 이동하는 동안 매초 여세를 <#e8a800ff>{0}중첩<> 얻으며, 최대 <#e8a800ff>{1}중첩<>까지 쌓입니다. 중첩당 <$speedIcon> <#ffffffff>이동 속도 {2}%<>를 얻습니다. 적 챔피언에게 <#ff9028ff>물리 피해<>를 입히면 남은 중첩을 모두 소모하여 소모한 여세에 비례해 <#ff9028ff>{3} - {4}의 추가 물리 피해<>를 입힙니다."
