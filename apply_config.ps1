...
$zekAura = "Aura: Grant <#d48294ff>{0}<> <$forceIcon> <#d48294ff>Adaptive Force<> to all allied champions within <#ff86c2ff>{1} <$rangeIcon> range<>, including yourself."
$i18n.en.zekes_herald.option = $zekAura -f $zekForce, $zekDist
$i18n.en.radiant_zekes_herald.option = $zekAura -f $rzekForce, $rzekDist
...
$litSuffering = "Suffering: Dealing Ability damage burns enemies, causing them to take <#d94c49ff>{0}% of the target's maximum health<> as <#a974ffff>magic damage<> over <#e8a800ff>{1} seconds<>. Deals a maximum of <#a974ffff>{2} magic damage<> per tick against minions and monsters."
...
$zekAuraVi = "Hào Quang: Cấp <#d48294ff>{0}<> <$forceIcon> <#d48294ff>Lực Thích Ứng<> cho mọi tướng đồng minh trong phạm vi <#ff86c2ff>{1}<> <$rangeIcon>, bao gồm cả bản thân."
$i18n.vi.zekes_herald.option = $zekAuraVi -f $zekForce, $zekDist
$i18n.vi.radiant_zekes_herald.option = $zekAuraVi -f $rzekForce, $rzekDist
...
$litSufferingVi = "Thống Khổ: Khi gây sát thương kĩ năng, thiêu đốt kẻ địch, khiến chúng chịu <#d94c49ff>{0}% máu tối đa của mục tiêu<> dưới dạng <#a974ffff>sát thương phép<> trong <#e8a800ff>{1} giây<>. Tối đa <#a974ffff>{2} sát thương phép<> mỗi nhịp lên lính và quái vật."
...
$zekAuraZh = "光环：为 <#ff86c2ff>{1} <$rangeIcon> 射程<>范围内的所有友方英雄提供 <#d48294ff>{0}<> <$forceIcon> <#d48294ff>自适应之力<>，包括你自己。"
$i18n.'zh-hans'.zekes_herald.option = $zekAuraZh -f $zekForce, $zekDist
$i18n.'zh-hans'.radiant_zekes_herald.option = $zekAuraZh -f $rzekForce, $rzekDist
...
$litSufferingZh = "苦难：造成技能伤害时点燃敌人，使其在 <#e8a800ff>{1}秒<>内受到相当于<#d94c49ff>目标{0}%最大生命值<>的<#a974ffff>魔法伤害<>。对小兵和野怪每跳最多造成 <#a974ffff>{2}点魔法伤害<>。"
...
$zekAuraPt = "Aura: Concede <#d48294ff>{0}<> <$forceIcon> de <#d48294ff>Força Adaptativa<> a todos os campeões aliados dentro de <#ff86c2ff>{1} <$rangeIcon> alcance<>, incluindo você mesmo."
$i18n.'pt-BR'.zekes_herald.option = $zekAuraPt -f $zekForce, $zekDist
$i18n.'pt-BR'.radiant_zekes_herald.option = $zekAuraPt -f $rzekForce, $rzekDist
...
$litSufferingPt = "Sofrimento: Causar dano de habilidade queima os inimigos, fazendo-os sofrer <#d94c49ff>{0}% da Vida Máxima do alvo<> como <#a974ffff>dano mágico<> ao longo de <#e8a800ff>{1} segundos<>. Máximo de <#a974ffff>{2} de dano mágico<> por tique contra tropas e monstros."
...
$zekAuraRu = "Аура: Даёт <#d48294ff>{0}<> <$forceIcon> <#d48294ff>адаптивной силы<> всем союзным чемпионам в пределах <#ff86c2ff>{1} <$rangeIcon> дальности<>, включая вас самих."
$i18n.ru.zekes_herald.option = $zekAuraRu -f $zekForce, $zekDist
$i18n.ru.radiant_zekes_herald.option = $zekAuraRu -f $rzekForce, $rzekDist
...
$litSufferingRu = "Страдание: Нанесение урона умением поджигает врагов, заставляя их получить <#d94c49ff>{0}% максимального здоровья цели<> в виде <#a974ffff>магического урона<> в течение <#e8a800ff>{1} секунд<>. Наносит максимум <#a974ffff>{2} магического урона<> за тик по миньонам и монстрам."
