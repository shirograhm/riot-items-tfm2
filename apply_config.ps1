...
$i18n.vi.stormrazor.option = "Tích Điện: Di chuyển và gây <#ff9028ff>sát thương vật lí<> tạo ra điểm <#92dc7bff>Tích Điện<>, tối đa <#e8a800ff>${srStacks}<>.`n`nTia Sét: Khi <#92dc7bff>Tích Điện<> đầy, đòn <#ff9028ff>sát thương vật lí<> tiếp theo gây <#a974ffff>${srDmg} sát thương phép cộng thêm<> và tăng <#ffffffff>${srMs}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<> trong <#e8a800ff>${srDur} giây<>."
$i18n.vi.radiant_stormrazor.option = "Tích Điện: Di chuyển và gây <#ff9028ff>sát thương vật lí<> tạo ra điểm <#92dc7bff>Tích Điện<>, tối đa <#e8a800ff>${rsrStacks}<>.`n`nTia Sét: Khi <#92dc7bff>Tích Điện<> đầy, đòn <#ff9028ff>sát thương vật lí<> tiếp theo gây <#a974ffff>${rsrDmg} sát thương phép cộng thêm<> và tăng <#ffffffff>${rsrMs}%<> <$speedIcon> <#ffffffff>tốc độ di chuyển<> trong <#e8a800ff>${rsrDur} giây<>."
...
$vcVi = "$lethVi`n`nTích Điện: Di chuyển và gây <#ff9028ff>sát thương vật lí<> tạo ra điểm <#92dc7bff>Tích Điện<>, tối đa <#e8a800ff>{1}<>.`n`nThiên Đường: Khi <#92dc7bff>Tích Điện<> đầy, đòn <#ff9028ff>sát thương vật lí<> tiếp theo cho bạn thêm <#ffdd8eff>{2} <$armorPenIcon> Xuyên Giáp Trắng<> trong <#e8a800ff>{3} giây<> và gây <#ff9028ff>sát thương vật lý cộng thêm<> tương đương <#d94c49ff>{4}% Máu hiện tại của mục tiêu<>. Tối đa <#ff9028ff>{5} sát thương vật lí<> lên lính và quái vật."
...
$i18n.'zh-hans'.stormrazor.option = "蓄能：移动和造成 <#ff9028ff>物理伤害<> 会产生<#92dc7bff>蓄能<>层数，最多 <#e8a800ff>${srStacks}<> 层。`n`n雷霆：<#92dc7bff>蓄能<>充满时，你的下一次 <#ff9028ff>物理伤害<> 额外造成 <#a974ffff>${srDmg} 点魔法伤害<>，并使你获得 <#ffffffff>${srMs}%<> <$speedIcon> <#ffffffff>移动速度<>，持续 <#e8a800ff>${srDur}秒<>。"
$i18n.'zh-hans'.radiant_stormrazor.option = "蓄能：移动和造成 <#ff9028ff>物理伤害<> 会产生<#92dc7bff>蓄能<>层数，最多 <#e8a800ff>${rsrStacks}<> 层。`n`n雷霆：<#92dc7bff>蓄能<>充满时，你的下一次 <#ff9028ff>物理伤害<> 额外造成 <#a974ffff>${rsrDmg} 点魔法伤害<>，并使你获得 <#ffffffff>${rsrMs}%<> <$speedIcon> <#ffffffff>移动速度<>，持续 <#e8a800ff>${rsrDur}秒<>。"
...
$vcZh = "$lethZh`n`n蓄能：移动和造成 <#ff9028ff>物理伤害<> 会产生<#92dc7bff>蓄能<>层数，最多 <#e8a800ff>{1}<> 层。`n`n苍穹：<#92dc7bff>蓄能<>充满时，你的下一次 <#ff9028ff>物理伤害<> 造成相当于目标<#d94c49ff>当前生命值{4}%<>的<#ff9028ff>额外物理伤害<>，并为你提供持续 <#e8a800ff>{3}秒<> 的 <#ffdd8eff>{2} 点<$armorPenIcon> 额外穿甲<>。对小兵和野怪最多造成 <#ff9028ff>{5}点物理伤害<>。"
...
$i18n.'pt-BR'.stormrazor.option = "Energizado: Mover-se e causar <#ff9028ff>dano físico<> gera acúmulos de <#92dc7bff>Energia<>, até <#e8a800ff>${srStacks}<>.`n`nRaio: Quando totalmente <#92dc7bff>Energizado<>, sua próxima instância de <#ff9028ff>dano físico<> causa <#a974ffff>${srDmg} de dano mágico bônus<> e concede <#ffffffff>${srMs}%<> de <$speedIcon> <#ffffffff>Velocidade de Movimento<> por <#e8a800ff>${srDur} segundos<>."
$i18n.'pt-BR'.radiant_stormrazor.option = "Energizado: Mover-se e causar <#ff9028ff>dano físico<> gera acúmulos de <#92dc7bff>Energia<>, até <#e8a800ff>${rsrStacks}<>.`n`nRaio: Quando totalmente <#92dc7bff>Energizado<>, sua próxima instância de <#ff9028ff>dano físico<> causa <#a974ffff>${rsrDmg} de dano mágico bônus<> e concede <#ffffffff>${rsrMs}%<> de <$speedIcon> <#ffffffff>Velocidade de Movimento<> por <#e8a800ff>${rsrDur} segundos<>."
...
$i18n.ko.stormrazor.option = "충전: 이동하거나 <#ff9028ff>물리 피해<>를 입히면 최대 <#e8a800ff>${srStacks}<>까지 <#92dc7bff>충전<> 중첩을 얻습니다.`n`n벼락: 완전히 <#92dc7bff>충전<>되면 다음 <#ff9028ff>물리 피해<>가 <#a974ffff>${srDmg}의 추가 마법 피해<>를 입히고, <#e8a800ff>${srDur}초<> 동안 <$speedIcon> <#ffffffff>이동 속도<>가 <#ffffffff>${srMs}%<> 증가합니다."
$i18n.ko.radiant_stormrazor.option = "충전: 이동하거나 <#ff9028ff>물리 피해<>를 입히면 최대 <#e8a800ff>${rsrStacks}<>까지 <#92dc7bff>충전<> 중첩을 얻습니다.`n`n벼락: 완전히 <#92dc7bff>충전<>되면 다음 <#ff9028ff>물리 피해<>가 <#a974ffff>${rsrDmg}의 추가 마법 피해<>를 입히고, <#e8a800ff>${rsrDur}초<> 동안 <$speedIcon> <#ffffffff>이동 속도<>가 <#ffffffff>${rsrMs}%<> 증가합니다."
...
$vcKo = "$lethKo`n`n충전: 이동하거나 <#ff9028ff>물리 피해<>를 입히면 최대 <#e8a800ff>{1}<>까지 <#92dc7bff>충전<> 중첩을 얻습니다.`n`n창공: 완전히 <#92dc7bff>충전<>되면 다음 <#ff9028ff>물리 피해<>가 대상 <#d94c49ff>현재 체력의 {4}%<>에 비례하는 <#ff9028ff>추가 물리 피해<>를 입히고 <#e8a800ff>{3}초<> 동안 <#ffdd8eff>{2}의 <$armorPenIcon> 추가 물리 관통력<>을 획득합니다. 미니언과 몬스터에게는 최대 <#ff9028ff>{5}의 물리 피해<>를 입힙니다."
