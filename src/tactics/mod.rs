...
fn mod_final_opts() -> Vec<(u64, String)> {
    ...
    if act.is_empty() { return all; }   // not scanned yet = undecidable -> no filter
    all.iter()
...
fn token_to_opt_index(tok: &str) -> Option<u8> {
    if tok == SEL_AUTO_TOKEN { return Some(SEL_AUTO); } // * explicit Auto (scoped keys only) - no registry needed
    if let Ok(n) = tok.parse::<u8>() {
        if n == 0 { return None; }
        if n < 7 { return Some(n); }                       // vanilla categories 1~6
        if !registry_ready() { return None; }              // legacy index but not resolvable yet
...
fn opt_index_to_token(idx: u8) -> Option<String> {
    ...
    if idx == SEL_AUTO { return Some(SEL_AUTO_TOKEN.to_string()); } // * explicit Auto must always be preserved
...
fn drain_pending(m: &mut HashMap<(String, u8), u8>) {
    ...
    for e in pend.iter_mut() { e.2 = normalize_token(&e.2); } // registry is ready -> pin numbers to keys
...
fn publish_ct_roster(blue: std::collections::HashSet<String>, red: std::collections::HashSet<String>) {
    ...
    if !cur.is_null() {
        let c = unsafe { &*cur };
        if c.0 == blue && c.1 == red { return; } // unchanged -> do not republish (bounded leak)
    }
...
fn ct_scope_for(champ: &str, side: u64) -> Scope {
    ...
    match (in_b, in_r) {
        ...
        (true, true) => { // same champion on both sides -> distinguish only by the learned side value
            if side != u64::MAX {
...
fn is_champ_designated(champ: &str) -> bool {
    ...
    match snap {
        ...
        None => { // first-time race (snapshot not built yet) -> force a build
...
fn hide_native_item_dds(root: &mut Node) {
    if !find_node(root, "personal").map(|n| n.visible).unwrap_or(false) { return; } // do not interfere unless on personal tactics
...
unsafe fn collect_slot3_icons(gv: usize) -> HashMap<(u64, u32), String> {
    ...
    for i in 0..=mask {
        ...
        if *((ctrl + i) as *const u8) & 0x80 != 0 { continue; } // not FULL
        ...
        if it_len < 4 || it_ptr < 0x10000 || !readable(it_ptr + 3 * 8, 8) { continue; } // does not own a 4th
                ...
                unsafe {
                    match tag {
                        ...
                        Some(t) => {
                            ...
                            if icon_tag_is(icon, t) { node_set_visible(icon, true); continue; } // same value = skip the rewrite
...
unsafe fn drive_slot3_tooltip(ui: &Node, gv: usize, hover: Option<(u64, u32, f32, f32, f32, f32)>) {
    let Some(tip) = find_node(ui, "item_tooltip") else { return };
    ...
    type TipShow = unsafe extern "win64" fn(
        usize, usize, usize, usize,   // p1, p2, p3 (measurement vtable constant), node
        usize, usize,                 // item_data, item_vtable  (borrowed only - never dropped)
...
fn handle_comptest_screen(ui: &Node) {
    ...
    if find_node(ui, "blue0").is_some() { CTD_ROW.fetch_add(1, Ordering::Relaxed); }
    ...
    for (ri, rid) in CT_ROWS.iter().enumerate() {
        let Some(row) = find_node(ui, rid) else { continue; };
        let Some(c) = ct_row_champ(row) else { champs[ri].clear(); continue; }; // no champion placed = not our business
...
unsafe extern "C" fn cap_launcher(saved: *mut u64, _e: usize) -> u64 {
    ...
    if (rva == 0x9a3287 || rva == 0x9a7b03 || is_comptest) && seed != 0 {
        ...
        if prev != seed { RENDER_PROVIDER.store(0, Ordering::Relaxed); } // new match seed -> the ctor right after re-captures the provider
...
fn install_launcher_hook() {
    ...
    if our != 0 && cur_tgt == our { CLAUNCH_INSTALLED.store(1, Ordering::Relaxed); HK_L_OURS.fetch_add(1, Ordering::Relaxed); return; } // entry point = our stub -> fine
    ...
    if !is_foreign && b0 != 0x48 && waited < 240 { HK_L_WAIT.fetch_add(1, Ordering::Relaxed); return; } // original prologue and still waiting -> wait for serpen to install
...
fn install_seed_ctor_hook() {
    if SEEDCTOR_INSTALLED.load(Ordering::Relaxed) == 1 { return; } // * skip only on 1 = success (0/2 = retry)
...
const SPAWN_PROLOGUE: [u8; 12] = [0x55, 0x41,0x57, 0x41,0x56, 0x41,0x55, 0x41,0x54, 0x56, 0x57, 0x53]; // 0.5.3: 8 push (12B) + sub rsp,0xf8 (0.5.2 was 7 push + mov eax,0x4d20)
...
fn install_spawn_hook() {
    if !SPAWN_INJECT_ENABLED { return; } // * when sealed, do not install the detour at all (a no-op hook = pure risk)
...
unsafe fn valid_ps_elem(elem: usize) -> bool {
    if read_str_try(elem + 0x420).is_some() { return true; } // the correct 0.5.0_3 position
...
unsafe fn install_detour_r11(rva: usize, orig_len: usize, cap_fn: usize, prologue: &[u8]) -> Result<usize, &'static str> {
    let base = GetModuleHandleW(core::ptr::null()) as usize;
    if base == 0 { return Err("module 0"); }
    if orig_len < 12 { return Err("orig_len<12"); } // the entry patch is 12B (movabs+jmp)
    let fn_addr = base + rva;
    if !readable(fn_addr, orig_len + 4) { return Err("unreadable"); }
...
fn tactics_post_update(client: &StableClient<'_>, in_game: bool) {
    {
        ...
        if BUILD_EXT_DIAG {
            ...
            if n % 300 == 0 {
                ...
                let buy_note = BUY_INSTALL_NOTE.lock().unwrap_or_else(|e| e.into_inner()).clone();
                s.push_str(&format!(
                    "\n-- hook install state --\n  \
        ...
        if let (true, Some(pid)) = (in_game, client.player_team_id()) {
            ...
            if pu == 0 {
                ...
                else { PID_ZERO_CLEAN.fetch_add(1, Ordering::Relaxed); } // an observation of 0 unrelated to comp test
...
fn tactics_on_server_start() { probe_db(); install_replace_4th(); install_launcher_hook(); install_seed_ctor_hook(); install_spawn_hook(); } // resolver = common to modes 3 and 4 (slot 0/1/2 designation) + the v13 identification hooks (launcher seed + seed-ctor provider)
...
fn stable_last_starting(client: &StableClient<'_>, team_id: usize) -> std::collections::HashSet<u64> {
...
fn stable_personal_tactics(
    client: &StableClient<'_>,
    team_id: usize,
) -> HashMap<String, [u8; 3]> {
    let mut out = HashMap::new();
    let Some(json) = client.record_get_json(RecordKindV1::Team, team_id, "champion_personal_tactics")
    else {
        return out;
    };
    ...
    for (champion, value) in entries {
        let JsonValue::Arr(categories) = value else { continue };
        if categories.len() < 3 {
            continue;
        }
        let mut slots = [0u8; 3];
...
fn probe_db() {
    ...
    if ITEM_NET_ADDR.load(Ordering::Relaxed) == 0 {
        unsafe {
            ...
            for &off in &[0x1558usize, 0xd30, 0xda0] { // * 0.5.1: prefer the game's real net = GameData+0x1558 (ghidra-re confirmed, identical in both versions). db == the GameData base.
                if sig_ok(db + off) { found = db + off; break; }
            }
            if found == 0 { // automatic window search (self-heals if it moves again in a future patch)
                let mut o = 0usize;
...
fn auto_cands() -> std::sync::Arc<Vec<u64>> {
    {
        ...
        if let Some(v) = g.as_ref() { return v.clone(); } // Arc clone = refcount only (no data copy)
    }
...
unsafe fn compute_auto_4th_id(athlete: usize, champ: &str) -> Option<u64> {
    ...
    let (ctx, real, vcount) = match build_lineup_ctx(athlete) {
        ...
        None => {
            let mut c = [0u64; 11];
            for k in 5..10 { c[k] = 9999; } // opponents unknown
            c[0] = cid; c[10] = 0;
            (c, false, 0)
        }
    };
...
fn slotN_item_key(scope: Scope, champ: &str, si: u8) -> Option<String> {
    ...
    mod_final_opts().get(idx as usize - 7).map(|(_, k)| k.clone()) // mod item
}
...
unsafe fn scan_idx_cached(ctx: usize, want: &[u8]) -> Option<u64> {
    ...
    { let mut g = SCAN_CACHE.lock().unwrap_or_else(|e| e.into_inner());
      if g.is_none() { *g = Some(HashMap::new()); }
      if let Some(outer) = g.as_mut() {
          if !outer.contains_key(&coll) && outer.len() >= 16 { outer.clear(); } // reset when too many collections (memory cap)
    ...
    let r = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> u64 {
        ...
        if SLOT012_INJECT_ENABLED && is_player {
            ...
            if ctx012 >= 0x10000 && bptr >= 0x10000 && blen >= 1 && blen <= 8 && readable(bptr, (blen as usize) * 8) {
                for si in 0u8..3 {
                    ...
                    if owned > si as u64 { continue; }         // slot already purchased -> too late
        ...
        if build_len == 3 && rd_u64(athlete + 0x480) == 3 { // 0.5.0 build cap (was 0x408)
            ...
            if ptr >= 0x10000 && readable(ptr, 24) && writable(athlete + 0x480, 0x18) {
                ...
                if t4.is_none() && BUILD_EXT_DIAG { BE_CNT[4].fetch_add(1, Ordering::Relaxed); } // failed to obtain the target index
                if let Some(t) = t4 {
        ...
        if DIAG_SCAN_OFF { return 0; } // * diagnostic #4: do only the realloc and skip the scan/shadow-call/forward (isolating realloc)
...
unsafe fn quick_scene_side(db: usize, pid: u64) -> Option<u64> {
    ...
    if t1_tag != 0 || t2_tag != 0 { return None; } // Normal (team_id) only
...
fn scene_player_side() -> Option<u64> {
    if !SCENE_GATE_ENABLED { return None; } // when OFF, use the roster fallback
