...
// ** 0.5.5 re-derivation (2026-08-11) - `tools/rederive.py loader` again, and it was needed: exe2exe `match`
//   returns **30 hits** here, all size-425 members of the same monomorphic clone family, so the masked
//   signature cannot pick between them. The string xref still can, and still converges: **16 of 16 sites on
//   0x2e42d0** (a .pdata function start, size 425 — the same size as 0.5.3/0.5.4), zero sites on anything else.
...
// ** 0.5.7 re-derivation (2026-08-26) - `tools/rederive.py loader` once more, and once more it was the only
//   thing that works here. The convergence is again total: **16 of 16 sites on 0x2ea930** (a .pdata function
//   start, size 425 — the same 425 it has been since 0.5.3), zero sites on anything else. player_info
//   lea@0x873989 and wide lea@0x8739cd both in fn 0x86e750; strategy lea@0x238601b, 0x238aad6 in fn 0x2385fc0;
...
const PARSER_RVA: usize = 0x197b90; // 0.6.0-beta2 (0.6.0-beta was 0x1ab1a0, 0.5.7 0x1ab310, 0.5.6 0x19ab40, 0.5.5 0x1a3e70, 0.5.4 0x1a3ce0, 0.5.3 0x1a6530, 0.5.2 0x24b5a00). The 3-argument contract (out, ptr, len), the `:`/`{`/`}` parsing, out[2]=-1 on error and the 0x90 node stride are all confirmed identical => NT_SIZE unchanged.
...
const ALLOC_RVA: usize  = 0x2f2f7e0; // 0.6.0-beta2 heap alloc helper (0.6.0-beta was 0x2dd4b50, 0.5.7 0x2ab4010, 0.5.6 0x2ab1670, 0.5.5 0x2a9bf30, 0.5.4 0x29bb920, 0.5.3 0x28f7df0). (corresponds to 0.5.2's 0x25d9640. The old __rust_alloc shim 0x25c4d30 does not exist in 0.5.3).
